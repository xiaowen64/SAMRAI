     ...
     d_precond_b_id(-1)
{
   ...
   d_FAC_solver = (solv::CellPoissonFACSolver<NDIM>*)NULL;
   ...
   d_nghosts = hier::IntVector<NDIM>(NUM_GHOSTS_U);

   d_solution = new pdat::CellVariable<NDIM,double>(d_object_name+"solution",1);

   int soln_id = variable_db->registerVariableAndContext(d_solution,
                                                         d_current,
                                                         hier::IntVector<NDIM>(0));
   if ( !viz_writer.isNull() ) {
      viz_writer->registerPlotScalar("U", soln_id);
   }
   ...
   d_weight = new pdat::CellVariable<NDIM,double>(d_object_name+"weight",1);
   ...
   d_source_term = new pdat::CellVariable<NDIM,double>(d_object_name+"source_term",1);
   ...
   d_exponential_term =
      new pdat::CellVariable<NDIM,double>(d_object_name+"exponential_term",1);
   ...
   d_diffusion_coef =
      new pdat::SideVariable<NDIM,double>(d_object_name+"diffusion_coef",1);

   int diffcoef_id = variable_db->registerVariableAndContext(d_diffusion_coef, 
                                                             d_scratch,
                                                             hier::IntVector<NDIM>(0));

   d_flux = new pdat::SideVariable<NDIM,double>(d_object_name+"flux",1);
   ...
   d_coarse_fine_flux =
      new pdat::OutersideVariable<NDIM,double>(d_object_name+"coarse_fine_flux",1);
   ...
   d_coarse_fine_flux_id = 
     ...
     */
    d_precond_a = new pdat::CellVariable<NDIM,double>(d_object_name+"precond_a",1);
    d_precond_a_id = variable_db->registerVariableAndContext(d_precond_a,
							     d_scratch,
							     hier::IntVector<NDIM>(0));
    d_precond_b = new pdat::FaceVariable<NDIM,double>(d_object_name+"precond_b",1);
     ...
     */
   d_jacobian_a =
      new pdat::CellVariable<NDIM,double>(d_object_name+":jacobian_a",1);
   d_jacobian_a_id = variable_db->registerVariableAndContext(d_jacobian_a, 
							     d_scratch,
							     hier::IntVector<NDIM>(0));
   d_jacobian_b =
      new pdat::FaceVariable<NDIM,double>(d_object_name+":jacobian_b",1);
   ...
   tbox::Pointer<CoarsenOperator<NDIM> > flux_coarsen_op =
   ...
   for ( int amr_level = hierarchy->getFinestLevelNumber();
      ...
      */
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
         ...
         tbox::Pointer<geom::CartesianPatchGeometry<NDIM> > patch_geometry =
	 ...
	 */
         ...
         tbox::Pointer<hier::PatchLevel<NDIM> > next_finer_level = 
            hierarchy->getPatchLevel(amr_level+1);
         hier::BoxArray<NDIM> coarsened_boxes = next_finer_level->getBoxes(); 
         hier::IntVector<NDIM> coarsen_ratio = next_finer_level->getRatio();
   ...
   const double old_dt) 
{
   ...
   if (first_step) {
      ...
      d_FAC_solver =
         new solv::CellPoissonFACSolver<NDIM>(d_object_name+":FAC_solver");
   ...
   if ( first_step ) {
      ...
      for (int amr_level = 0;
         ...
         for (hier::PatchLevel<NDIM>::Iterator p(patch_level); p; p++) {
            ...
            tbox::Pointer< pdat::CellData<NDIM,double> > y_cur = 
               patch->getPatchData(d_solution, d_current);
            tbox::Pointer< pdat::CellData<NDIM,double> > y_new = 
            ...
            tbox::Pointer< pdat::SideData<NDIM,double> > diffusion =
               patch->getPatchData(d_diffusion_coef, d_scratch);

#if (NDIM == 1)
            FORT_EVALDIFFUSION(ifirst(0), ilast(0),
                               dx, xlo, xhi,
...
bool ModifiedBratuProblem::checkNewSolution(const int solver_retcode)
{
   ...
   for (int amr_level = 0;
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(patch_level); p; p++) {
         ...
         const hier::Index<NDIM> ifirst = patch->getBox().lower();
         const hier::Index<NDIM> ilast = patch->getBox().upper();
         tbox::Pointer<geom::CartesianPatchGeometry<NDIM> > patch_geom = patch->getPatchGeometry();
         const double* dx = patch_geom->getDx();
         const double* xlo = patch_geom->getXLower();
         const double* xhi = patch_geom->getXUpper();
         tbox::Pointer< pdat::CellData<NDIM,double> > u = patch->getPatchData(d_solution, d_new);
         tbox::Pointer< pdat::CellData<NDIM,double> > w = patch->getPatchData(d_weight_id);

#if (NDIM == 1)
         FORT_ERROR( ifirst(0), ilast(0),
   ...
   tbox::pout << " At " << new_time << " err is " << maxerror << endl;
...
void ModifiedBratuProblem::updateSolution(const double new_time)
{
   tbox::Pointer<hier::PatchHierarchy<NDIM> > hierarchy = d_solution_vector->getPatchHierarchy();
    ...
    */
   {
      ...
      for ( ln=ln_beg; ln<ln_end; ++ln ) {
         ...
         d_flux_coarsen_schedule[ln] =
            d_flux_coarsen.createSchedule(level, finer_level);
      }
   }
   {
      ...
      for ( ln=ln_beg; ln<=ln_end; ++ln ) {
         tbox::Pointer<hier::PatchLevel<NDIM> > level =
            patch_hierarchy->getPatchLevel(ln);
         d_soln_fill_schedule[ln] =
            d_soln_fill.createSchedule(level,
                                       ln-1,
                                       patch_hierarchy,
                                       (RefinePatchStrategy<NDIM> *)this);
      }
   }
   {
      const int ln_beg = coarsest_level - (coarsest_level>0);
      ...
      for ( ln=ln_beg; ln<ln_end; ++ln ) {
         ...
         d_soln_coarsen_schedule[ln] =
            d_soln_coarsen.createSchedule(level, finer_level);
         d_scratch_soln_coarsen_schedule[ln] =
            d_scratch_soln_coarsen.createSchedule(level, finer_level);
      }
   }
   {
      const int ln_beg = coarsest_level - (coarsest_level>0);
   ...
   Vec fcur)
{
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > x =
      solv::PETSc_SAMRAIVectorReal<NDIM,double>::getSAMRAIVector(xcur);
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > f =
      solv::PETSc_SAMRAIVectorReal<NDIM,double>::getSAMRAIVector(fcur);
   ...
   Vec x)
{
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > xvec =
      solv::PETSc_SAMRAIVectorReal<NDIM,double>::getSAMRAIVector(x);
   ...
   Vec xout)
{
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > xinvec =
      solv::PETSc_SAMRAIVectorReal<NDIM,double>::getSAMRAIVector(xin);
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > xoutvec =
      solv::PETSc_SAMRAIVectorReal<NDIM,double>::getSAMRAIVector(xout);
...
int ModifiedBratuProblem::setupPreconditioner(Vec x)
{
  tbox::Pointer<solv::SAMRAIVectorReal<NDIM,double> > uvec =
    solv::PETSc_SAMRAIVectorReal<NDIM,double>::getSAMRAIVector(x);
   ...
   Vec z)
{
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > rhs =
      solv::PETSc_SAMRAIVectorReal<NDIM,double>::getSAMRAIVector(r);
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > soln =
      solv::PETSc_SAMRAIVectorReal<NDIM,double>::getSAMRAIVector(z);
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > f)
{
    ...
    */
   CoarsenAlgorithm<NDIM> eval_average;
   ...
   for (amr_level = hierarchy->getFinestLevelNumber()-1;
        ...
        amr_level-- ) {
      tbox::Pointer<hier::PatchLevel<NDIM> > level       = hierarchy->getPatchLevel(amr_level);
      tbox::Pointer<hier::PatchLevel<NDIM> > finer_level = hierarchy->getPatchLevel(amr_level+1);
   ...
   for (amr_level = hierarchy->getFinestLevelNumber();
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
         ...
         tbox::Pointer< pdat::OutersideData<NDIM,double> > coarse_fine_flux =
            patch->getPatchData(d_coarse_fine_flux_id);
#if (NDIM == 1)
         FORT_EVALFACEFLUXES( ifirst(0), ilast(0),
                              NUM_GHOSTS_U,
                              diffusion->getPointer(0),
                              u->getPointer(),
                              dx,
         ...
         for (int i = 0; i < bdry_faces.getSize(); i++) {
            ...
            hier::Box<NDIM> bbox = bdry_faces[i].getBox();
            const hier::Index<NDIM> ibeg = bbox.lower();
            const hier::Index<NDIM> iend = bbox.upper(); 
            int face = bdry_faces[i].getLocationIndex();

#if (NDIM == 1)
            FORT_EWBCFLUXFIX( ifirst(0), ilast(0),
                              NUM_GHOSTS_U,
                              dx,
                              u->getPointer(),
                              flux->getPointer(0),
                              bbox.lower(), bbox.upper(),
                              face );
#endif
#if (NDIM == 2)
            FORT_EWBCFLUXFIX( ifirst(0), ilast(0),
                              ...
                              bbox.lower(), bbox.upper(),
                              face );
            FORT_NSBCFLUXFIX( ifirst(0), ilast(0),
                              ifirst(1), ilast(1),
                              NUM_GHOSTS_U,
                              dx,
                              u->getPointer(),
            ...
            FORT_NSBCFLUXFIX( ifirst(0), ilast(0),
                              ...
                              bbox.lower(), bbox.upper(),
                              face );
            FORT_TBBCFLUXFIX( ifirst(0), ilast(0),
                              ifirst(1), ilast(1),
                              ifirst(2), ilast(2),
                              NUM_GHOSTS_U,
                              dx,
         ...
         if ( amr_level > 0 ) {
            ...
            for (int side = 0; side < 2; side++) {
#if (NDIM == 1)
               FORT_EWFLUXCOPY( ifirst(0), ilast(0),
                                flux->getPointer(0),
               ...
               FORT_EWFLUXCOPY( ifirst(0), ilast(0),
                                ...
                                coarse_fine_flux->getPointer(0,side),
                                side );
               FORT_NSFLUXCOPY( ifirst(0), ilast(0),
                                ifirst(1), ilast(1),
                                flux->getPointer(1),
               ...
               FORT_NSFLUXCOPY( ifirst(0), ilast(0),
                                ...
                                coarse_fine_flux->getPointer(1,side),
                                side );
               FORT_TBFLUXCOPY( ifirst(0), ilast(0),
                                ifirst(1), ilast(1),
                                ifirst(2), ilast(2),
                                flux->getPointer(2),
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
         ...
         tbox::Pointer< pdat::CellData<NDIM,double> > u =
            patch->getPatchData(d_solution, d_scratch); 
         tbox::Pointer< pdat::CellData<NDIM,double> > u_cur =
            patch->getPatchData(d_solution, d_current); 
         tbox::Pointer< pdat::CellData<NDIM,double> > source = 
            patch->getPatchData(d_source_term, d_scratch);
         tbox::Pointer< pdat::CellData<NDIM,double> > exponential = 
            patch->getPatchData(d_exponential_term, d_scratch);
         tbox::Pointer< pdat::SideData<NDIM,double> > flux =
            patch->getPatchData(d_flux_id);
         tbox::Pointer< pdat::CellData<NDIM,double> > fcur = 
            f->getComponentPatchData(0, *patch);
         tbox::Pointer< pdat::CellData<NDIM,double> > xdat = 
            x->getComponentPatchData(0, *patch);
#if (NDIM == 1)
         FORT_EVALEXPONENTIAL(ifirst(0), ilast(0),
                              xdat->getPointer(),
                              d_lambda,
...
#endif
#if (NDIM == 1)
         FORT_EVALSOURCE( ifirst(0), ilast(0),
                          d_lambda,
...
#endif
#if (NDIM == 1)
         FORT_EVALBRATU( ifirst(0), ilast(0),
                         NUM_GHOSTS_U,
                         flux->getPointer(0),
                         source->getPointer(),
                         exponential->getPointer(),
   ...
   CoarsenAlgorithm<NDIM> f_average;
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > Jv )
{
  ...
  TBOX_ASSERT(!(Jv   == (tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> >)NULL));
   ...
   CoarsenAlgorithm<NDIM> jacv_average;
   ...
   tbox::Pointer<hier::PatchHierarchy<NDIM> > hierarchy = Jv->getPatchHierarchy();
    ...
    */
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
         ...
         tbox::Pointer<geom::CartesianPatchGeometry<NDIM> > patch_geom =
           patch->getPatchGeometry();
         const double* dx = patch_geom->getDx();
         tbox::Pointer< pdat::CellData<NDIM,double> > vdat =
    ...
    */
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
         ...
         tbox::Pointer< pdat::CellData<NDIM,double> > jac_a =
            patch->getPatchData(d_jacobian_a_id);
         tbox::Pointer< pdat::CellData<NDIM,double> > Jvdat =
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > x)
{
   ...
   for (int amr_level = hierarchy->getFinestLevelNumber();
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
         ...
         tbox::Pointer< pdat::CellData<NDIM,double> > a = patch->getPatchData(d_precond_a_id);
	 ...
	 TBOX_ASSERT( !b.isNull() );
          ...
          */
#if (NDIM == 1)
         compfacoffdiag_(ifirst(0),ilast(0),
                         d_current_dt,
                         cell_vol,
                         diffusion->getPointer(0),
                         b->getPointer(0));
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > z)
{
    ...
    * Then, cycle through the levels and coarsen the data.
    */
   CoarsenAlgorithm<NDIM> pc_rhs_average;
   ...
   math::HierarchyCellDataOpsReal<NDIM,double>
      math_ops(hierarchy,
               0,
    ...
    * Then, cycle through the levels and coarsen the data.
    */
   CoarsenAlgorithm<NDIM> pc_sol_average;
   ...
   for (int amr_level = hierarchy->getFinestLevelNumber()-1;
      ...
      d_scratch_soln_coarsen.resetSchedule(d_scratch_soln_coarsen_schedule[amr_level]);
   }
   ...
   for (int amr_level = hierarchy->getFinestLevelNumber();
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
         ...
         tbox::Pointer< pdat::CellData<NDIM,double> > src_data =
            patch->getPatchData(d_soln_scratch_id);
         tbox::Pointer< pdat::CellData<NDIM,double> > dst_data = patch->getPatchData(z_indx);
   ...
   tbox::Pointer< solv::SAMRAIVectorReal<NDIM,double> > x)
{
   ...
   for (int amr_level = hierarchy->getFinestLevelNumber();
      ...
      for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
         ...
         tbox::Pointer< pdat::CellData<NDIM,double> > a = patch->getPatchData(d_jacobian_a_id);
         tbox::Pointer< pdat::FaceData<NDIM,double> > b = patch->getPatchData(d_jacobian_b_id);
          ...
          */
	 math::PatchCellDataOpsReal<NDIM,double> cell_basic_ops;
   ...
   const hier::IntVector<NDIM>& ghost_width_to_fill )
{
   ...
   */
   ...
   tbox::Pointer< pdat::CellData<NDIM,double> > u = patch.getPatchData(d_soln_scratch_id);
   ...
   for (int i=0; i<boundary.getSize(); i++) {
      hier::Box<NDIM> bbox = hier::Box<NDIM>(boundary[i].getBox());
      face = boundary[i].getLocationIndex();
#if (NDIM == 1)
      FORT_SETBC( ifirst(0), ilast(0),
                  NUM_GHOSTS_U,
                  u->getPointer(),
                  bbox.lower(), bbox.upper(),
                  face );
...
void ModifiedBratuProblem::printClassData(ostream& os) const
{
   os << "\nModifiedBratuProblem::printClassData..." << endl;
   os << "ModifiedBratuProblem: this = " << (ModifiedBratuProblem*)this << endl;
...
void ModifiedBratuProblem::getLevelEdges(hier::BoxList<NDIM>& boxes, 
...
void ModifiedBratuProblem::correctLevelFlux(tbox::Pointer<hier::PatchLevel<NDIM> > level)
{
   for (hier::PatchLevel<NDIM>::Iterator p(level); p; p++) {
      tbox::Pointer<hier::Patch<NDIM> > patch = level->getPatch(p());
      const hier::Box<NDIM> box = patch->getBox();
      tbox::Pointer< pdat::SideData<NDIM,double> > flux_data = patch->getPatchData(d_flux_id);
      ...
      for (int d = 0; d < NDIM; d++) {
         for (int s = 0; s <= 1; s++) {
            hier::Index<NDIM> delta(0);
            delta(d) = ( (s == 0) ? 1 : -1 );
            hier::Index<NDIM> twodelta(0);
            twodelta(d) = ( (s == 0) ? 2 : -2 ); 
            hier::BoxList<NDIM> level_edges;
            getLevelEdges(level_edges, patch, level, d, s);
            for (hier::BoxList<NDIM>::Iterator l(level_edges); l; l++) {
               for (pdat::CellIterator<NDIM> ic(l()); ic; ic++) {
                  pdat::SideIndex<NDIM> iside(ic() + delta, d, s);
                      ...
                      tbox::Pointer< pdat::CellData<NDIM,double> > u )
{
   ...
   for (int d = 0; d < NDIM; d++) {
      for (int s = 0; s <= 1; s++) {
         hier::Index<NDIM> delta1(0);
         delta1(d) = ( (s == 0) ? 1 : -1 );
         hier::Index<NDIM> delta2(0);
         delta2(d) = ( (s == 0) ? 2 : -2 );
         ...
         for (hier::BoxList<NDIM>::Iterator l(level_edges); l; l++) {
            for (pdat::CellIterator<NDIM> ic(l()); ic; ic++) {
               pdat::SideIndex<NDIM> iside(ic() + delta1, d, s);
