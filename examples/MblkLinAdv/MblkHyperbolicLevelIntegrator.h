...
/**
 * Class MblkHyperbolicLevelIntegrator<DIM> provides routines needed to 
 ...
 * the TimeRefinementIntegrator<DIM> class.  Together, that hierarchy 
 ...
 * base class HyperbolicPatchStrategy<DIM>.
 *
 ...
 * TimeRefinementLevelStrategy<DIM>, which defines routines needed by
 ...
 * TimeRefinementLevelStrategy<DIM> are: initializeLevelIntegrator(),
 ...
 * This class is also derived from mesh::StandardTagAndInitStrategy<DIM>,
 * which defines routines needed by the gridding algorithm classes.  The
 * routines overloaded in mesh::StandardTagAndInitStrategy<DIM> are:
...
class MblkHyperbolicLevelIntegrator : 
   public TimeRefinementLevelStrategy<NDIM>,
...
public:
   ...
   /**
    * Constructor for MblkHyperbolicLevelIntegrator<DIM> initializes
   ...
   /**
    * The destructor for MblkHyperbolicLevelIntegrator<DIM> unregisters 
    ...
    *
    * Assertion checking will throw unrecoverable exceptions if either 
    * pointer is null.
    */
   virtual void
    ...
    */
   virtual void
   synchronizeNewLevels(const tbox::Pointer< hier::BasePatchHierarchy<NDIM> > hierarchy,
                        const int coarsest_level,
                        const int finest_level,
                        const double sync_time,
                        const bool initial_time);
    ...
    */
   virtual void
   resetTimeDependentData(const tbox::Pointer< hier::BasePatchLevel<NDIM> > level,
                          const double new_time,
    ...
    */
   virtual void
   resetDataToPreadvanceState(const tbox::Pointer< hier::BasePatchLevel<NDIM> > level);
    ...
    */
   virtual void 
   applyRichardsonExtrapolation(const tbox::Pointer< hier::PatchLevel<NDIM> > level,
                                const double error_data_time,
                                const int tag_index,
                                const double deltat,
    ...
    */
   virtual void 
   registerVariable(const tbox::Pointer< hier::Variable<NDIM> > var, 
                    const hier::IntVector<NDIM> ghosts, 
                    const HYP_VAR_TYPE h_v_type,
                    const tbox::Pointer<xfer::CoarsenOperator<NDIM> > coarsen_op = NULL,
                    const tbox::Pointer<xfer::RefineOperator<NDIM> > refine_op = NULL,
                    const tbox::Pointer<xfer::TimeInterpolateOperator<NDIM> > time_int = NULL);
    ...
    * in the concrete HyperbolicPatchStrategy<NDIM> object manipulate;
...
protected:
    ...
    */
   virtual void
   preprocessFluxData(const tbox::Pointer< hier::MultiblockPatchLevel<NDIM> > level,
    ...
    */
   virtual void
   postprocessFluxData(const tbox::Pointer< hier::MultiblockPatchLevel<NDIM> > level,
                       const bool regrid_advance,
                       const bool first_step,
                       const bool last_step);
    ...
    * the levels.   
    *
    * When assertion checking is turned on, an unrecoverable exception
    ...
    */
   virtual void
   synchronizeLevelWithCoarser(const tbox::Pointer< hier::MultiblockPatchLevel<NDIM> > fine,
                               const tbox::Pointer< hier::MultiblockPatchLevel<NDIM> > coarse,
                               const double sync_time,
 ...
 */
    ...
    */
   tbox::Pointer< xfer::MultiblockRefineAlgorithm<NDIM> > 
    ...
    * MblkHyperbolicLevelIntegrator<DIM> second occurs when the dt calculation is
    ...
    * accompanying MblkHyperbolicLevelIntegrator<DIM>::advanceLevel.
    */
   tbox::Pointer< xfer::MultiblockRefineAlgorithm<NDIM> > 
    ...
    */
   tbox::Pointer< xfer::MultiblockCoarsenAlgorithm<NDIM> > 
    ...
    * the HyperbolicPatchStrategy<DIM> object.  The variable lists
