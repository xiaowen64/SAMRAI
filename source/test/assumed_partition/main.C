   ...
   char* argv[])
{
   ...
   while (true) {
      ...
      boost::shared_ptr<tbox::Database> test_db =
         input_db->getDatabaseWithDefault(test_name,
      ...
      if (test_fail_count) {
         tbox::pout << "FAILED: selfCheck found " << fail_count
                    << " problems in test " << test_name
                    << " (" << nickname << ')' << std::endl;
      }
   ...
   tbox::Database &test_db)
{
   ...
   ctp.index_begin =
      test_db.getIntegerWithDefault("index_begin", ctp.index_begin);
   ctp.avg_parts_per_rank =
      test_db.getDoubleWithDefault("avg_parts_per_rank",
                                   ctp.avg_parts_per_rank);
   return ctp;
}
