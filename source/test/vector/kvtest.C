...
int main( int argc, char *argv[] ) {
   ...
   // tbox::PIO::logAllNodes("kvtest.log");
   ...
   assert(coarse_domain_list.getNumberItems() == 1);
   assert(fine_level_list.getNumberItems() == 1);
#endif
   ...
   // Construct SAMRAI vector wrappers to test operations via PVodeTrio calls
   N_Vector kvec0 = solv::PVodeTrio_SAMRAIVector<3>::createPVodeTrioVector(my_vec0);
   N_Vector kvec1 = solv::PVodeTrio_SAMRAIVector<3>::createPVodeTrioVector(my_vec1);
   ...
   if (!tbox::Utilities::deq(my_norm, p_norm)) {
   ...
   if (!tbox::Utilities::deq(my_min_val, p_min_val)) {
   ...
   if (!tbox::Utilities::deq(my_max_val, p_max_val)) {
   ...
   if (!tbox::Utilities::deq(max_val, (double)1100.0)) {
      tbox::perr << "FAILED: - Test #11, max bogus value\n";
   }

   double min_val = my_vec1->min();
   if (!tbox::Utilities::deq(min_val, (double)-3300.0)) {
   ...
   if (!tbox::Utilities::deq(my_dot, p_dot)) {
   ...
   if (!tbox::Utilities::deq(norm, (double)7.6393717)) {
   ...
   if (!tbox::Utilities::deq(norm, (double)12.0)) {
      tbox::perr << "FAILED: - Test #18, RMS norm calculation\n";
   }

   norm = my_vec0->weightedRMSNorm(my_vec1);
   if (!tbox::Utilities::deq(norm, (double)5.77482219887084)) {
   ...
   N_Vector kvec2 = solv::PVodeTrio_SAMRAIVector<3>::createPVodeTrioVector(my_vec2);

   tbox::plog << "\n\nPRINTING VARIABLE DATABASE before adding new vector" << endl;
   variable_db->printClassData(tbox::plog);

   N_Vector kvec3 = N_VNew(0, kvec2);

   tbox::Pointer< solv::SAMRAIVectorReal<3,double> > sam_vec3 =
      solv::PVodeTrio_SAMRAIVector<3>::getSAMRAIVector(kvec3);
   ...
   N_VConst(two, kvec1);
   tbox::plog << N_VConstrProdPos(kvec0, kvec2) << " = 0" << endl;
   tbox::plog << N_VConstrProdPos(kvec1, kvec2) << " = 1" << endl;
   ...
   // Destroy PVodeTrio vector wrappers
   solv::PVodeTrio_SAMRAIVector<3>::destroyPVodeTrioVector(kvec0);
   solv::PVodeTrio_SAMRAIVector<3>::destroyPVodeTrioVector(kvec1);
   solv::PVodeTrio_SAMRAIVector<3>::destroyPVodeTrioVector(kvec2);
