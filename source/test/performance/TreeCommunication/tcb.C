...
unsigned long dummy() { static unsigned long i = 0; i = (i+1)%1000; return i; }
   ...
   char* argv[])
{
...

   {
      ...
      boost::shared_ptr<tbox::Timer> t_child_wait = TimerManager::getManager()->getTimer("apps::main::child_wait");
      TimerManager::getManager()->getTimer("apps::main::parent_send");
      TimerManager::getManager()->getTimer("apps::main::parent_recv");
      boost::shared_ptr<tbox::Timer> t_parent_wait = TimerManager::getManager()->getTimer("apps::main::parent_wait");
      TimerManager::getManager()->getTimer("apps::main::equiv_MPI");
      boost::shared_ptr<tbox::Timer> t_processing = TimerManager::getManager()->getTimer("apps::main::processing");
   ...
   Database &test_db )
{
   ...
   cts.barrier_after_each_repetition = test_db.getBoolWithDefault("barrier_after_each_repetition", false);
   ...
   Database &test_db )
{
   ...
   boost::shared_ptr<Timer> repetitions_timer = TimerManager::getManager()->getTimer("apps::main::repetitions");
   boost::shared_ptr<Timer> processing_timer = TimerManager::getManager()->getTimer("apps::main::processing");
   boost::shared_ptr<Timer> verify_timer = TimerManager::getManager()->getTimer("apps::main::verify");
   boost::shared_ptr<Timer> equiv_mpi_timer = TimerManager::getManager()->getTimer("apps::main::equiv_MPI");
...
void simulateDataProcessing( const CommonTestSwitches &cts )
{
   ...
   unsigned long microsecs =
      static_cast<unsigned long>(randf*(cts.processing_cost[0] + cts.msg_length*cts.processing_cost[1]));
   my_usleep(microsecs);
}
   ...
   const CommonTestSwitches &cts )
{
   ...
   parent_stage.setCommunicationWaitTimer( TimerManager::getManager()->getTimer("apps::main::parent_wait") );
   child_stage.setCommunicationWaitTimer( TimerManager::getManager()->getTimer("apps::main::child_wait") );
