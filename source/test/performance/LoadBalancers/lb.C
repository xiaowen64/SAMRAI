...

   {
...

      {
         ...
         const int boxes_per_proc =
            (anchor_boxes.size() + anchor_box_level.getMPI().getSize()
             - 1) / anchor_box_level.getMPI().getSize();
   ...
   hier::Connector& balance_to_anchor)
{
   ...
   anchor_box_level_ptr(&anchor_box_level, false);

   tbox::Pointer<hier::PatchLevel> tag_level(
      new hier::PatchLevel(anchor_box_level,
         grid_geometry,
