//
// File:        side_cplxtest.C
// Package:     SAMRAI test
// Copyright:   (c) 1997-2005 The Regents of the University of California
// Revision:    $Revision: 590 $
// Modified:    $Date: 2005-08-29 10:37:19 -0700 (Mon, 29 Aug 2005) $
...
int main( int argc, char *argv[] ) {
   ...
   tbox::PIO::logAllNodes("side_cplxtest.log");
   ...
   tbox::Pointer< pdat::SideVariable<2,dcomplex> > fvar[NVARS];
   int svindx[NVARS];
   fvar[0] = new pdat::SideVariable<2,dcomplex>("fvar0", 1);
   svindx[0] = variable_db->registerVariableAndContext(
               fvar[0], dummy, no_ghosts);
   fvar[1] = new pdat::SideVariable<2,dcomplex>("fvar1", 1);
   svindx[1] = variable_db->registerVariableAndContext(
               fvar[1], dummy, no_ghosts);
   fvar[2] = new pdat::SideVariable<2,dcomplex>("fvar2", 1);
   svindx[2] = variable_db->registerVariableAndContext(
               fvar[2], dummy, no_ghosts);
   fvar[3] = new pdat::SideVariable<2,dcomplex>("fvar3", 1);
   svindx[3] = variable_db->registerVariableAndContext(
               fvar[3], dummy, no_ghosts);

   tbox::Pointer< pdat::SideVariable<2,double> >
   swgt = new pdat::SideVariable<2,double>("swgt", 1);
   ...
   tbox::Pointer<math::HierarchyDataOpsComplex<2> > side_ops = 
   new math::HierarchySideDataOpsComplex<2>(hierarchy, 0, 1);
   assert(!side_ops.isNull());

   tbox::Pointer<math::HierarchyDataOpsReal<2,double> > swgt_ops = 
   new math::HierarchySideDataOpsReal<2,double>(hierarchy, 0 , 1);
   ...
   for (ln = 0; ln < 2; ln++) {
      ...
      for (hier::PatchLevel<2>::Iterator ip(level); ip; ip++) {
         tbox::Pointer< pdat::SideData<2,double> > data;
         ...
         if (ln == 0) {
            ...
            if (patch->getPatchNumber() == 0) {
               ...
               for (ic = plo0; ic <= phi0; ic++) {
                  fi = pdat::SideIndex<2>(hier::Index<2>(ic,plo1), pdat::SideIndex<2>::Y, pdat::SideIndex<2>::Lower);
                  (*data)(fi) *= 0.5;
               }
               //left and right side boundaries
               for (ic = plo1; ic <= phi1; ic++) {
                  fi = pdat::SideIndex<2>(hier::Index<2>(plo0,ic), pdat::SideIndex<2>::X, pdat::SideIndex<2>::Lower);
                  (*data)(fi) *= 0.5;
                  fi = pdat::SideIndex<2>(hier::Index<2>(phi0,ic), pdat::SideIndex<2>::X, pdat::SideIndex<2>::Upper);
                  (*data)(fi) *= 0.5;
               }
            } else {
               ...
               for (ic = plo0; ic <= phi0; ic++) {
                  ...
                  (*data)(fi) = 0.0;
                  fi = pdat::SideIndex<2>(hier::Index<2>(ic,phi1), pdat::SideIndex<2>::Y, pdat::SideIndex<2>::Upper);
                  (*data)(fi) *= 0.5;
               }
               //left and right side boundaries
               for (ic = plo1; ic <= phi1; ic++) {
         ...
         for (pdat::CellIterator<2> c(svdata->getBox());c && vol_test_passed;c++) {
            ...
            if (ln == 0) {
               if ((coarse_fine * patch->getBox()).contains(cell_index)) {
                  if ( !tbox::Utilities::deq((*svdata)(cell_index),0.0) ) {
                     vol_test_passed = false;
                  }
               } else {
                  if ( !tbox::Utilities::deq((*svdata)(cell_index),0.01) ) {
                     vol_test_passed = false;
                  }
               }
            }
            ...
            if (ln == 1) {
               if ( !tbox::Utilities::deq((*svdata)(cell_index),0.0025) ) {
                  vol_test_passed = false;
               }
            }
         }
...
*/
   ...
   if (!complexDataSameAsValue(svindx[2],val_scale,hierarchy)) {
      tbox::perr << "FAILED: - Test #6: math::HierarchySideDataOpsComplex<2>::scale()\n"
           << "Expected: v2 = " << val_scale << endl;
      side_ops->printData(svindx[2], tbox::plog);
   }

   // Test #7: math::HierarchySideDataOpsComplex<2>::add()
   ...
   if (!complexDataSameAsValue(svindx[3],val_add,hierarchy)) {
      tbox::perr << "FAILED: - Test #7: math::HierarchySideDataOpsComplex<2>::add()\n"
   ...
   // Test #8: math::HierarchySideDataOpsComplex<2>::subtract()
   ...
   if (!complexDataSameAsValue(svindx[1],val_sub,hierarchy)) {
      tbox::perr << "FAILED: - Test #8: math::HierarchySideDataOpsComplex<2>::subtract()\n"
           << "Expected: v1 = " << val_sub << endl;
      side_ops->printData(svindx[1], tbox::plog);
   }

   // Test #9a: math::HierarchySideDataOpsComplex<2>::addScalar()
   ...
   if (!complexDataSameAsValue(svindx[1],val_addScalar,hierarchy)) {
      tbox::perr << "FAILED: - Test #9a: math::HierarchySideDataOpsComplex<2>::addScalar()\n"
           << "Expected: v1 = " << val_addScalar << endl;
      side_ops->printData(svindx[1], tbox::plog);
   }

   // Test #9b: math::HierarchySideDataOpsComplex<2>::addScalar()
   ...
   if (!complexDataSameAsValue(svindx[2],val_addScalar,hierarchy)) {
      tbox::perr << "FAILED: - Test #9b: math::HierarchySideDataOpsComplex<2>::addScalar()\n"
           << "Expected: v2 = " << val_addScalar << endl;
      side_ops->printData(svindx[2], tbox::plog);
   }

   // Test #9c: math::HierarchySideDataOpsComplex<2>::addScalar()
   // Expected:  v2 = v2 + (3.0,-4.0) = (4.0,-3.0)
   side_ops->addScalar(svindx[2], svindx[2], dcomplex(3.0,-4.0));
   val_addScalar = dcomplex(4.0,-3.0);
   if (!complexDataSameAsValue(svindx[2],val_addScalar,hierarchy)) {
      tbox::perr << "FAILED: - Test #9c: math::HierarchySideDataOpsComplex<2>::addScalar()\n"
           << "Expected: v2 = " << val_addScalar << endl;
      side_ops->printData(svindx[2], tbox::plog);
   }
   ...
   // Test #10: math::HierarchySideDataOpsComplex<2>::multiply()
   ...
   if (!complexDataSameAsValue(svindx[1],val_mult,hierarchy)) {
      tbox::perr << "FAILED: - Test #10: math::HierarchySideDataOpsComplex<2>::multiply()\n"
           << "Expected: v1 = " << val_mult << endl;
      side_ops->printData(svindx[1], tbox::plog);
   }

   // Test #11: math::HierarchySideDataOpsComplex<2>::divide()
   ...
   if (!complexDataSameAsValue(svindx[0],val_div,hierarchy)) {
      tbox::perr << "FAILED: - Test #11: math::HierarchySideDataOpsComplex<2>::divide()\n"
           << "Expected: v0 = " << val_div << endl;
      side_ops->printData(svindx[0], tbox::plog);
   }

   // Test #12: math::HierarchySideDataOpsComplex<2>::reciprocal()
   ...
   if (!complexDataSameAsValue(svindx[1],val_rec,hierarchy)) {
      tbox::perr << "FAILED: - Test #12: math::HierarchySideDataOpsComplex<2>::reciprocal()\n"
           << "Expected: v1 = " << val_rec << endl;
      side_ops->printData(svindx[1], tbox::plog);
   }

   // Test #13:  Place some bogus values on coarse level
   tbox::Pointer< pdat::SideData<2,dcomplex> > sdata;
   ...
   for (hier::PatchLevel<2>::Iterator ip(level_zero); ip; ip++) {
      ...
      if (patch->getBox().contains(index0)) {
         (*sdata)(pdat::SideIndex<2>(index0, pdat::SideIndex<2>::Y, pdat::SideIndex<2>::Lower), 0) = dcomplex(100.0,-50.0);
      }
      if (patch->getBox().contains(index1)) {
         (*sdata)(pdat::SideIndex<2>(index1, pdat::SideIndex<2>::Y, pdat::SideIndex<2>::Upper), 0) = dcomplex(-1000.0,20.0);
      }
   }
   ...
   for (hier::PatchLevel<2>::Iterator ipp(level_zero); ipp; ipp++) {
      ...
      sdata = patch->getPatchData(svindx[2]);
      pdat::SideIndex<2> index0(hier::Index<2>(2,2),pdat::SideIndex<2>::Y,pdat::SideIndex<2>::Lower);
      pdat::SideIndex<2> index1(hier::Index<2>(5,3),pdat::SideIndex<2>::Y,pdat::SideIndex<2>::Upper);

      // check X axis data
      for (pdat::SideIterator<2> c(sdata->getBox(),pdat::SideIndex<2>::X);c && bogus_value_test_passed;c++) {
         pdat::SideIndex<2> side_index = c();

         if (!tbox::Utilities::ceq((*sdata)(side_index),
            dcomplex(4.0,-3.0)) ) {
      ...
      // check Y axis data
      for (pdat::SideIterator<2> cc(sdata->getBox(),pdat::SideIndex<2>::Y);cc && bogus_value_test_passed;cc++) {
         pdat::SideIndex<2> side_index = cc();
   ...
   // Test #14: math::HierarchySideDataOpsComplex<2>::L1Norm() - w/o control weight
   // Expected:  bogus_l1_norm = 2217.003379
   double bogus_l1_norm = side_ops->L1Norm(svindx[2]);
   if ( !tbox::Utilities::deq(bogus_l1_norm,2217.003379) ) {
      tbox::perr << "FAILED: - Test #14: math::HierarchySideDataOpsComplex<2>::L1Norm()"
           << " - w/o control weight\n"
           << "Expected value = 2217.003379, Computed value = "
           << setprecision(12) << bogus_l1_norm << endl;
   }

   // Test #15: math::HierarchySideDataOpsComplex<2>::L1Norm() - w/control weight
   // Expected:  correct_l1_norm = 5.0
   double correct_l1_norm = side_ops->L1Norm(svindx[2],swgt_id);
   if ( !tbox::Utilities::deq(correct_l1_norm,5.0) ) {
      tbox::perr << "FAILED: - Test #15: math::HierarchySideDataOpsComplex<2>::L1Norm()"
           << " - w/control weight\n"
           << "Expected value = 5.0, Computed value = "
           << correct_l1_norm << endl;
   }

   // Test #16: math::HierarchySideDataOpsComplex<2>::L2Norm()
   // Expected:  l2_norm = 5.0
   double l2_norm = side_ops->L2Norm(svindx[2],swgt_id);
   if ( !tbox::Utilities::deq(l2_norm,5.0) ) {
      tbox::perr << "FAILED: - Test #16: math::HierarchySideDataOpsComplex<2>::L2Norm()\n"
           << "Expected value = 5.0, Computed value = "
           << l2_norm << endl;
   }

   // Test #17: math::HierarchySideDataOpsComplex<2>::maxNorm() - w/o control weight
   // Expected:  bogus_max_norm = 1000.19998
   double bogus_max_norm = side_ops->maxNorm(svindx[2]);
   if ( !tbox::Utilities::deq(bogus_max_norm,1000.19998) ) {
      tbox::perr << "FAILED: - Test #17: math::HierarchySideDataOpsComplex<2>::maxNorm()"
           << " - w/o control weight\n"
           << "Expected value = 1000.19998, Computed value = "
           << bogus_max_norm << endl;
   }

   // Test #18: math::HierarchySideDataOpsComplex<2>::maxNorm() - w/control weight
   // Expected:  max_norm = 5.0
   double max_norm = side_ops->maxNorm(svindx[2],swgt_id);
   if ( !tbox::Utilities::deq(max_norm,5.0) ) {
      tbox::perr << "FAILED: - Test #18: math::HierarchySideDataOpsComplex<2>::maxNorm()"
           << " - w/control weight\n"
           << "Expected value = 5.0, Computed value = "
           << max_norm << endl;
   }
   ...
   // Test #19: math::HierarchySideDataOpsComplex<2>::linearSum()
   // Expected:  v3 = (2.0,5.0)
   side_ops->linearSum(svindx[3], dcomplex(2.0,0.0), svindx[1], dcomplex(0.0, -1.0), svindx[0]);
   dcomplex val_linearSum(2.0,5.0);
   if (!complexDataSameAsValue(svindx[3],val_linearSum,hierarchy)) {
      tbox::perr << "FAILED: - Test #19: math::HierarchySideDataOpsComplex<2>::linearSum()\n"
           << "Expected: v3 = " << val_linearSum << endl;
      side_ops->printData(svindx[3], tbox::plog);
   }

   // Test #20: math::HierarchySideDataOpsComplex<2>::axmy()
   ...
   if (!complexDataSameAsValue(svindx[3],val_axmy,hierarchy)) {
      tbox::perr << "FAILED: - Test #20: math::HierarchySideDataOpsComplex<2>::axmy()\n"
           << "Expected: v3 = " << val_axmy << endl;
      side_ops->printData(svindx[3], tbox::plog);
   }

   // Test #21a: math::HierarchySideDataOpsComplex<2>::dot() - (ind2) * (ind1)
   // Expected:  cdot = (17.5,-21.0)
   dcomplex cdot = side_ops->dot(svindx[2], svindx[1], swgt_id);
   dcomplex ans_2_dot_1(17.5,-21.0);
   if ( !tbox::Utilities::ceq(cdot,ans_2_dot_1) ) {
      tbox::perr << "FAILED: - Test #21a: math::HierarchySideDataOpsComplex<2>::dot() - (ind2) * (ind1)\n"
           << "Expected value = (17.5,-21.0), Computed value = "
           << cdot << endl;
   }

   // Test #21b: math::HierarchySideDataOpsComplex<2>::dot() - (ind1) * (ind2)
   // Expected:  cdot = (17.5,-1.0)
   dcomplex cdot2 = side_ops->dot(svindx[1], svindx[2], swgt_id); 
   dcomplex ans_1_dot_2(17.5,21.0);
   if ( !tbox::Utilities::ceq(cdot2,ans_1_dot_2) ) {
      tbox::perr << "FAILED: - Test #21b: math::HierarchySideDataOpsComplex<2>::dot() - (ind1) * (ind2)\n"
           << "Expected value = (17.5,21.0), Computed value = "
           << cdot2 << endl;
   }

   // Test #22: math::HierarchySideDataOpsComplex<2>::abs()
   // Expected:  abs(v0) = 5.0
   ...
   if (!doubleDataSameAsValue(swgt_id,5.0,hierarchy)) {
      tbox::perr << "FAILED: - Test #22: math::HierarchySideDataOpsComplex<2>::abs()\n"
   ...
   tbox::Pointer<hier::PatchHierarchy<2> > hierarchy)
{
   ...
   for (ln = 0; ln < 2; ln++) {
      ...
      for (hier::PatchLevel<2>::Iterator ip(level); ip; ip++) {
         patch = level->getPatch(ip());
         tbox::Pointer< pdat::SideData<2,dcomplex> > svdata = patch->getPatchData(desc_id);

         for (pdat::SideIterator<2> c(svdata->getBox(),1); c && test_passed ; c++) {
            pdat::SideIndex<2> side_index = c();
            if ( !tbox::Utilities::ceq((*svdata) (side_index),value) ) {
               test_passed = false;
            }
         }
      }
   ...
   tbox::Pointer<hier::PatchHierarchy<2> > hierarchy)
{
   ...
   for (ln = 0; ln < 2; ln++) {
      ...
      for (hier::PatchLevel<2>::Iterator ip(level); ip; ip++) {
         patch = level->getPatch(ip());
         tbox::Pointer< pdat::SideData<2,double> > svdata = patch->getPatchData(desc_id);

         for (pdat::SideIterator<2> c(svdata->getBox(),1); c && test_passed ; c++) {
            pdat::SideIndex<2> side_index = c();
            if ( !tbox::Utilities::deq((*svdata) (side_index),value) ) {
               test_passed = false;
            }
         }
      }
