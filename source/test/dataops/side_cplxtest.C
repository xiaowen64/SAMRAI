//
// File:        $URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-2-0/source/test/dataops/side_cplxtest.C $
// Package:     SAMRAI test
   ...
   for (ln = 0; ln < 2; ln++) {
      ...
      for (hier::PatchLevel<2>::Iterator ip(level); ip; ip++) {
         ...
         for (pdat::SideIterator<2> c(svdata->getBox(),1); c && test_passed ; c++) {
            pdat::SideIndex<2> side_index = c();
            if ( !tbox::MathUtilities<dcomplex>::equalEps((*svdata) (side_index),value) ) {
               test_passed = false;
            }
         }
      }
		      ...
		      tbox::Pointer<hier::PatchHierarchy<2> > hierarchy)
{
   ...
   for (ln = 0; ln < 2; ln++) {
      ...
      for (hier::PatchLevel<2>::Iterator ip(level); ip; ip++) {
         ...
         for (pdat::SideIterator<2> c(svdata->getBox(),1); c && test_passed ; c++) {
            pdat::SideIndex<2> side_index = c();
            if ( !tbox::MathUtilities<double>::equalEps((*svdata) (side_index),value) ) {
               test_passed = false;
            }
         }
      }
