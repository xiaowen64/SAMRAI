...
// tbox::PIO::logOnlyNodeZero("node_hiertest.log");
   tbox::PIO::logAllNodes("node_hiertest.log");
         ...
         for (pdat::NodeIterator<2> c(cvdata->getBox());c && vol_test_passed;c++) {
            ...
            if (ln == 0) {
               if ((coarse_fine * patch->getBox()).contains(node_index)) {
                  if ( !tbox::Utilities::deq((*cvdata)(node_index),0.0) ) {
                     vol_test_passed = false;
                  }
               } else {
                  if ( !tbox::Utilities::deq((*cvdata)(node_index),0.01) ) {
                     vol_test_passed = false;
                  }
               }
            }
            ...
            if (ln == 1) {
               if ( !tbox::Utilities::deq((*cvdata)(node_index),0.0025) ) {
                  vol_test_passed = false;
               }
            }
         }
...
*/
   ...
   if (!doubleDataSameAsValue(nvindx[2],val_scale,hierarchy)) {
      tbox::perr << "FAILED: - Test #6: math::HierarchyNodeDataOpsReal<2>::scale()\n"
           << "Expected: v2 = " << val_scale << endl;
      node_ops->printData(nvindx[2], tbox::plog);
   }

   // Test #7: math::HierarchyNodeDataOpsReal<2>::add()
   // Expected: v3 = v0 + v1 = (6.0) 
   node_ops->add(nvindx[3], nvindx[0], nvindx[1]);
   double val_add = 6.0;
   if (!doubleDataSameAsValue(nvindx[3],val_add,hierarchy)) {
      tbox::perr << "FAILED: - Test #7: math::HierarchyNodeDataOpsReal<2>::add()\n"
   ...
   // Test #8: math::HierarchyNodeDataOpsReal<2>::subtract()
   // Expected: v1 = v3 - v0 = (6.0)
   node_ops->subtract(nvindx[1], nvindx[3], nvindx[0]);  
   double val_sub = 6.0;
   if (!doubleDataSameAsValue(nvindx[1],val_sub,hierarchy)) {
      tbox::perr << "FAILED: - Test #8: math::HierarchyNodeDataOpsReal<2>::subtract()\n"
           << "Expected: v1 = " << val_sub << endl;
      node_ops->printData(nvindx[1], tbox::plog);
   }

   // Test #9a: math::HierarchyNodeDataOpsReal<2>::addScalar()
   ...
   if (!doubleDataSameAsValue(nvindx[1],val_addScalar,hierarchy)) {
      tbox::perr << "FAILED: - Test #9a: math::HierarchyNodeDataOpsReal<2>::addScalar()\n"
           << "Expected: v1 = " << val_addScalar << endl;
      node_ops->printData(nvindx[1], tbox::plog);
   }

   // Test #9b: math::HierarchyNodeDataOpsReal<2>::addScalar()
   // Expected: v2 = v2 + (0.0) = (1.0) 
   node_ops->addScalar(nvindx[2], nvindx[2], 0.0); 
   val_addScalar = 1.0;
   if (!doubleDataSameAsValue(nvindx[2],val_addScalar,hierarchy)) {
      tbox::perr << "FAILED: - Test #9b: math::HierarchyNodeDataOpsReal<2>::addScalar()\n"
           << "Expected: v2 = " << val_addScalar << endl;
      node_ops->printData(nvindx[2], tbox::plog);
   }

   // Test #9c: math::HierarchyNodeDataOpsReal<2>::addScalar()
   // Expected:  v2 = v2 + (3.0) = (4.0)
   node_ops->addScalar(nvindx[2], nvindx[2], 3.0);
   val_addScalar = 4.0;
   if (!doubleDataSameAsValue(nvindx[2],val_addScalar,hierarchy)) {
      tbox::perr << "FAILED: - Test #9c: math::HierarchyNodeDataOpsReal<2>::addScalar()\n"
           << "Expected: v2 = " << val_addScalar << endl;
      node_ops->printData(nvindx[2], tbox::plog);
   }
   ...
   // Test #10: math::HierarchyNodeDataOpsReal<2>::multiply()
   // Expected:  v1 = v3 * v1 = (3.0) 
   node_ops->multiply(nvindx[1], nvindx[3], nvindx[1]);
   double val_mult = 3.0;
   if (!doubleDataSameAsValue(nvindx[1],val_mult,hierarchy)) {
      tbox::perr << "FAILED: - Test #10: math::HierarchyNodeDataOpsReal<2>::multiply()\n"
           << "Expected: v1 = " << val_mult << endl;
      node_ops->printData(nvindx[1], tbox::plog);
   }

   // Test #11: math::HierarchyNodeDataOpsReal<2>::divide()
   ...
   if (!doubleDataSameAsValue(nvindx[0],val_div,hierarchy)) {
      tbox::perr << "FAILED: - Test #11: math::HierarchyNodeDataOpsReal<2>::divide()\n"
           << "Expected: v0 = " << val_div << endl;
      node_ops->printData(nvindx[0], tbox::plog);
   }

   // Test #12: math::HierarchyNodeDataOpsReal<2>::reciprocal()
   ...
   if (!doubleDataSameAsValue(nvindx[1],val_rec,hierarchy)) {
      tbox::perr << "FAILED: - Test #12: math::HierarchyNodeDataOpsReal<2>::reciprocal()\n"
           << "Expected: v1 = " << val_rec << endl;
      node_ops->printData(nvindx[1], tbox::plog);
   }

   // Test #13: math::HierarchyNodeDataOpsReal<2>::abs()
   // Expected:  v3 = abs(v2) = 4.0
   node_ops->abs(nvindx[3], nvindx[2]);
   double val_abs = 4.0;
   if (!doubleDataSameAsValue(nvindx[3],val_abs,hierarchy)) {
      tbox::perr << "FAILED: - Test #13: math::HierarchyNodeDataOpsReal<2>::abs()\n"
   ...
   for (hier::PatchLevel<2>::Iterator ipp(level_zero); ipp; ipp++) {
      ...
      for (pdat::NodeIterator<2> c(ndata->getBox());c && bogus_value_test_passed;c++) {
         ...
         if (node_index == index0) {
            if (!tbox::Utilities::deq((*ndata)(node_index),100.0)) {
               bogus_value_test_passed = false;
            }
         } else {
            if (node_index == index1) {
               if (!tbox::Utilities::deq((*ndata)(node_index),-1000.0)) {
                  bogus_value_test_passed = false;
               }
            } else {
               if (!tbox::Utilities::deq((*ndata)(node_index), 4.0)) {
                  bogus_value_test_passed = false;
               }
            }
         }
   ...
   // Test #15: math::HierarchyNodeDataOpsReal<2>::L1Norm() - w/o control weight
   // Expected:  bogus_l1_norm = 1640.00
   double bogus_l1_norm = node_ops->L1Norm(nvindx[2]);
   if ( !tbox::Utilities::deq(bogus_l1_norm,1640.00) ) {
      tbox::perr << "FAILED: - Test #15: math::HierarchyNodeDataOpsReal<2>::L1Norm()"
           << " - w/o control weight\n"
           << "Expected value = 1640.00, Computed value = "
           << setprecision(12) << bogus_l1_norm << endl;
   }

   // Test #16: math::HierarchyNodeDataOpsReal<2>::L1Norm() - w/control weight
   // Expected:  correct_l1_norm = 2.0
   double correct_l1_norm = node_ops->L1Norm(nvindx[2],nwgt_id);
   if ( !tbox::Utilities::deq(correct_l1_norm,2.0) ) {
      tbox::perr << "FAILED: - Test #16: math::HierarchyNodeDataOpsReal<2>::L1Norm()"
           << " - w/control weight\n"
           << "Expected value = 2.0, Computed value = "
           << correct_l1_norm << endl;
   }

   // Test #17: math::HierarchyNodeDataOpsReal<2>::L2Norm()
   // Expected:  l2_norm = 2.8284271
   double l2_norm = node_ops->L2Norm(nvindx[2],nwgt_id);
   if ( !tbox::Utilities::deq(l2_norm,2.82842712475) ) {
      tbox::perr << "FAILED: - Test #17: math::HierarchyNodeDataOpsReal<2>::L2Norm()\n"
           << "Expected value = 2.82842712475, Computed value = "
           << l2_norm << endl;
   }

   // Test #18: math::HierarchyNodeDataOpsReal<2>::maxNorm() - w/o control weight
   // Expected:  bogus_max_norm = 1000.0
   double bogus_max_norm = node_ops->maxNorm(nvindx[2]);
   if ( !tbox::Utilities::deq(bogus_max_norm,1000.0) ) {
      tbox::perr << "FAILED: - Test #18: math::HierarchyNodeDataOpsReal<2>::maxNorm()"
           << " - w/o control weight\n"
           << "Expected value = 1000.0, Computed value = "
           << bogus_max_norm << endl;
   }

   // Test #19: math::HierarchyNodeDataOpsReal<2>::maxNorm() - w/control weight
   // Expected:  max_norm = 4.0
   double max_norm = node_ops->maxNorm(nvindx[2],nwgt_id);
   if ( !tbox::Utilities::deq(max_norm,4.0) ) {
      tbox::perr << "FAILED: - Test #19: math::HierarchyNodeDataOpsReal<2>::maxNorm()"
           << " - w/control weight\n"
   ...
   // Test #20: math::HierarchyNodeDataOpsReal<2>::linearSum()
   // Expected:  v3 = 5.0
   ...
   if (!doubleDataSameAsValue(nvindx[3],val_linearSum,hierarchy)) {
      tbox::perr << "FAILED: - Test #20: math::HierarchyNodeDataOpsReal<2>::linearSum()\n"
           << "Expected: v3 = " << val_linearSum << endl;
      node_ops->printData(nvindx[3], tbox::plog);
   }
  
   // Test #21: math::HierarchyNodeDataOpsReal<2>::axmy()
   // Expected:  v3 = 6.5
   node_ops->axmy(nvindx[3], 3.0, nvindx[1], nvindx[0]);
   double val_axmy = 6.5;
   if (!doubleDataSameAsValue(nvindx[3],val_axmy,hierarchy)) {
      tbox::perr << "FAILED: - Test #21: math::HierarchyNodeDataOpsReal<2>::axmy()\n"
           << "Expected: v3 = " << val_axmy << endl;
      node_ops->printData(nvindx[3], tbox::plog);
   }

   // Test #22a: math::HierarchyNodeDataOpsReal<2>::dot() - (ind2) * (ind1)
   // Expected:  cdot = 8.75
   double cdot = node_ops->dot(nvindx[2], nvindx[1], nwgt_id);
   if ( !tbox::Utilities::deq(cdot,8.75) ) {
      tbox::perr << "FAILED: - Test #22a: math::HierarchyNodeDataOpsReal<2>::dot() - (ind2) * (ind1)\n"
           << "Expected Value = 8.75, Computed Value = "
           << cdot << endl;
   }

   // Test #22a: math::HierarchyNodeDataOpsReal<2>::dot() - (ind1) * (ind2)
   // Expected:  cdot = 8.75
   cdot = node_ops->dot(nvindx[1], nvindx[2], nwgt_id); 
   if ( !tbox::Utilities::deq(cdot,8.75) ) {
      tbox::perr << "FAILED: - Test #22a: math::HierarchyNodeDataOpsReal<2>::dot() - (ind1) * (ind2)\n"
           << "Expected Value = 8.75, Computed Value = "
           << cdot << endl;
   }
   ...
   for (ln = 0; ln < 2; ln++) {
      ...
      for (hier::PatchLevel<2>::Iterator ip(level); ip; ip++) {
         ...
         for (pdat::NodeIterator<2> c(nvdata->getBox()); c && test_passed ; c++) {
            pdat::NodeIndex<2> node_index = c();
            if ( !tbox::Utilities::deq((*nvdata) (node_index),value) ) {
               test_passed = false;
            }
         }
      }
