//
// File:        $URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-2-0/source/test/dataops/edge_hiertest.C $
// Package:     SAMRAI test
...
*/
	 ...
	 for (pdat::EdgeIterator<2> c(cdata->getBox(),pdat::EdgeIndex<2>::X);c && bogus_value_test_passed;c++) {
	    ...
	    if (!tbox::MathUtilities<double>::equalEps((*cdata)(edge_index), 4.0)) {
	 ...
	 for (pdat::EdgeIterator<2> cc(cdata->getBox(),pdat::EdgeIndex<2>::Y);cc && bogus_value_test_passed;cc++) {
	    ...
	    if (edge_index == index0) {
	       if (!tbox::MathUtilities<double>::equalEps((*cdata)(edge_index),100.0)) {
		  bogus_value_test_passed = false;
	       }
	    } else {
	       if (edge_index == index1) {
		  if (!tbox::MathUtilities<double>::equalEps((*cdata)(edge_index),-1000.0)) {
		     bogus_value_test_passed = false;
		  }
	       } else {
		  if (!tbox::MathUtilities<double>::equalEps((*cdata)(edge_index), 4.0)) {
   ...
   for (ln = 0; ln < 2; ln++) {
	 ...
	 patch = level->getPatch(ip());
         ...
         for (pdat::EdgeIterator<2> c(cvdata->getBox(),1); c && test_passed ; c++) {
            pdat::EdgeIndex<2> edge_index = c();
            if ( !tbox::MathUtilities<double>::equalEps((*cvdata) (edge_index),value) ) {
               test_passed = false;
            }
         }
      }
