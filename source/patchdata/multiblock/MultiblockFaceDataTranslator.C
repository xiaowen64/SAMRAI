//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/trunk/source/multiblock/MultiblockFaceDataTranslator.C $
...
MultiblockFaceDataTranslator<DIM,TYPE>::MultiblockFaceDataTranslator()
{
}
...
template <int DIM, class TYPE>
void MultiblockFaceDataTranslator<DIM,TYPE>::translateAndCopyData(
      ...
      hier::MultiblockPatchHierarchy<DIM>::RotationIdentifier rotate)
{
   ...
   tbox::Pointer< FaceData<DIM,TYPE> > dst = dst_patch.getPatchData(dst_id);
   tbox::Pointer< FaceData<DIM,TYPE> > src = src_patch.getPatchData(src_id);
...
template<int DIM, class TYPE>
void MultiblockFaceDataTranslator<DIM,TYPE>::translateAndCopyArrayData(
