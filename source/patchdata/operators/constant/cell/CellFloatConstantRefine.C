//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-3-0/source/patchdata/operators/constant/cell/CellFloatConstantRefine.C $
...
extern "C" {
// in conrefine1d.f:
   void conrefcellflot1d_( const int&, const int&,
                           const int&, const int&,
                           const int&, const int&,
                           const int&, const int&,
                           const int*,
                           const float*, float* );
// in conrefine2d.f:
   void conrefcellflot2d_( const int&, const int&, const int&, const int&,
                           ...
                           const float*, float* );
// in conrefine3d.f:
   void conrefcellflot3d_( const int&, const int&, const int&,
...
template<int DIM> CellFloatConstantRefine<DIM>::CellFloatConstantRefine()
...
template<int DIM> bool CellFloatConstantRefine<DIM>::findRefineOperator(
...
CellFloatConstantRefine<DIM>::getOperatorName() const
{
   return(d_name_id);
}

template<int DIM> int CellFloatConstantRefine<DIM>::getOperatorPriority() const
{
   return(0);
}

template<int DIM> hier::IntVector<DIM> 
CellFloatConstantRefine<DIM>::getStencilWidth() const {
   return(hier::IntVector<DIM>(0));
}

template<int DIM> void CellFloatConstantRefine<DIM>::refine(
   hier::Patch<DIM>& fine, 
...
{
   tbox::Pointer< CellData<DIM,float> >
   ...
   for (int d = 0; d < fdata->getDepth(); d++) {
      if (DIM == 1) {
	 conrefcellflot1d_(ifirstc(0),ilastc(0),
			   ifirstf(0),ilastf(0),
			   cilo(0),cihi(0),
			   filo(0),fihi(0),
			   ratio,
	 ...
	 conrefcellflot2d_(ifirstc(0),ifirstc(1),ilastc(0),ilastc(1),
	 ...
	 conrefcellflot3d_(ifirstc(0),ifirstc(1),ifirstc(2),
	 ...
	 TBOX_ERROR("CellFloatConstantRefine::refine DIM > 3 not supported" << std::endl);
      }
   }
}
