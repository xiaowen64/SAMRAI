//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-2-0/source/patchdata/operators/time_interpolate/cell/CellComplexLinearTimeInterpolateOp.C $
...
extern "C" {
// in lintimint1d.f:
   void lintimeintcellcmplx1d_( const int&, const int&,
                                const int&, const int&,
                                const int&, const int&,
                                const int&, const int&,
                                const double&,
                                ...
                                dcomplex* );
// in lintimint2d.f:
   void lintimeintcellcmplx2d_( const int&, const int&,
                                const int&, const int&,
                                const int&, const int&,
                                const int&, const int&,
                                const int&, const int&,
                                ...
                                dcomplex* );
// in lintimint3d.f:
   void lintimeintcellcmplx3d_( const int&, const int&, const int&,
...
template<int DIM> CellComplexLinearTimeInterpolateOp<DIM>::CellComplexLinearTimeInterpolateOp()
...
template<int DIM> bool CellComplexLinearTimeInterpolateOp<DIM>::findTimeInterpolateOperator(
   const tbox::Pointer< hier::Variable<DIM> >& var,
   const std::string &op_name) const
{
   const tbox::Pointer< CellVariable<DIM,dcomplex> > cast_var(var);
...
template<int DIM> void CellComplexLinearTimeInterpolateOp<DIM>::timeInterpolate(
   ...
   const hier::PatchData<DIM>& src_data_new) const
{
   ...
   const CellData<DIM,dcomplex> *old_dat =
      dynamic_cast<const CellData<DIM,dcomplex> *>(&src_data_old);
   const CellData<DIM,dcomplex> *new_dat =
      dynamic_cast<const CellData<DIM,dcomplex> *>(&src_data_new);
   CellData<DIM,dcomplex> *dst_dat =
      dynamic_cast<CellData<DIM,dcomplex> *>(&dst_data);
   ...
   for (int d = 0; d < dst_dat->getDepth(); d++) {
      if (DIM == 1) {
	 lintimeintcellcmplx1d_(ifirst(0),ilast(0),
				old_ilo(0),old_ihi(0),
				new_ilo(0),new_ihi(0),
				dst_ilo(0),dst_ihi(0),
				tfrac,
	 ...
	 lintimeintcellcmplx2d_(ifirst(0),ifirst(1),ilast(0),ilast(1),
	 ...
	 lintimeintcellcmplx3d_(ifirst(0),ifirst(1),ifirst(2),
	 ...
	 TBOX_ERROR("CellComplexLinearTimeInterpolateOp::TimeInterpolate DIM > 3 not supported" << std::endl);
      }
