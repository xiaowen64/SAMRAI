...
#include "SAMRAI/appu/EmbeddedBoundaryGeometry.I"
#endif

namespace SAMRAI {
namespace appu {

const int EmbeddedBoundaryGeometry::APPU_EMBEDDED_BOUNDARY_GEOMETRY_VERSION = 1;
const int EmbeddedBoundaryGeometry::CUTCASE_UNDEFINED = -1;
...
EmbeddedBoundaryGeometry::EmbeddedBoundaryGeometry(
   ...
   d_ebdry_data_id(Bdry::UNDEFINED),
   d_cell_flag_data_id(Bdry::UNDEFINED),
   d_cell_vol_data_id(Bdry::UNDEFINED),
   d_node_flag_data_id(Bdry::UNDEFINED),
   d_max_levels(0),
   ...
   const tbox::Pointer<hier::PatchLevel> old_level)
{
   ...
   if (d_read_ebdry) {
      ...
      int number_cells = 0;
      const hier::BoxList& level_boxes = level->getBoxes();
      for (hier::BoxList::Iterator i(level_boxes); i; i++) {
         number_cells += i().size();
      }
   ...
   double& max_area_error)
{
   ...
   **************************************************************/
    ...
    */
   int i;
   const hier::BoxList& domain_boxes =
      level->getGridGeometry()->getPhysicalDomain(0);
   hier::Index domain_ilo(dim, tbox::MathUtilities<int>::getMax());
   for (hier::BoxList::Iterator n(domain_boxes); n; n++) {
      for (i = 0; i < dim.getValue(); i++) {
         if (n().lower(i) < domain_ilo(i)) {
            domain_ilo[i] = n().lower(i);
         }
      }
   }
   ...
   const double* dx)
{
   ...
   if (d_compute_areas_and_normal) {
      ...
      for (int j = 0; j < num_bdry_nodes; j++) {
         ...
         double dist = Bdry::UNDEFINED;
         double distsq = Bdry::UNDEFINED;
   ...
   double& error_estimate) const
{
    ...
    */
   int face_dim = Bdry::UNDEFINED;
   int face_side = Bdry::UNDEFINED;
   if (face == 0) {
      face_dim = 0;
      face_side = 0;
   } else if (face == 1) {
   ...
   const std::string& dirname) const
{
    ...
    */
   const hier::BoxList& patch_boxes = level->getBoxes();
   ...
   const std::string& dirname) const
{
    ...
    */
   hier::BoxList patch_boxes(db->getDatabaseBoxArray("patch_boxes"));
   bool same = true;
   hier::BoxList::Iterator itr(patch_boxes);
   for (int p = 0; p < level->getNumberOfPatches(); p++, itr++) {
      if (p >= patch_boxes.getNumberOfBoxes()) {
         same = false;
         break;
      }
      const hier::Box& rbox = *itr;
      hier::MappedBoxId mbid(hier::LocalId(p), pid, hier::BlockId::zero());
      const hier::Box& pbox = level->getPatch(mbid)->getBox();
      if (rbox != pbox) {
         same = false;
         break;
      }
