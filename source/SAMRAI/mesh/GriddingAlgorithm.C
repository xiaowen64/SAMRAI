...
tbox::Pointer<tbox::Timer> GriddingAlgorithm::t_fill_tags;
   ...
   const hier::MappedBoxLevel& override_mapped_box_level)
{
    ...
    * balanced MappedBoxLevel to itself.  Since domain is small, owned
    ...
    * scalable because we use the domain MappedBoxLevel in bridges and
   ...
   hier::MappedBoxLevel new_mapped_box_level(
      domain_mapped_box_level.getMappedBoxes(),
   ...
   if (!level_zero_exists) {
       ...
       * bridging across the domain MappedBoxLevel is probably not
      ...
      if (do_tagging) {
         ...
         if (new_mapped_box_level.isInitialized()) {
            ...
            if (s_check_proper_nesting == 'y') {
               /*
                * Check that the new MappedBoxLevel nests inside the tag level.
                *
 ...
 *************************************************************************
 */
void GriddingAlgorithm::regridFinerLevel_doTaggingBeforeRecursiveRegrid(
   const int tag_ln,
   const bool level_is_coarsest_sync_level,
   const tbox::Array<double> &regrid_start_time,
   const double regrid_time)
{
   const hier::IntVector& zero_vec(hier::IntVector::getZero(d_dim));
   const tbox::Pointer<hier::PatchLevel> &tag_level(d_hierarchy->getPatchLevel(tag_ln));
 ...
 *************************************************************************
 */
void GriddingAlgorithm::regridFinerLevel_doTaggingAfterRecursiveRegrid(
 ...
 *************************************************************************
 */
void GriddingAlgorithm::regridFinerLevel_createAndInstallNewLevel(
   const int tag_ln,
   const double regrid_time,
   ...
   const hier::Connector &finer_to_tag)
{
   ...
   const int new_ln = tag_ln + 1;
   const tbox::Pointer<hier::PatchLevel> &tag_level = d_hierarchy->getPatchLevel(tag_ln);
   const hier::MappedBoxLevel &new_mapped_box_level(tag_to_new.getHead());
      ...
      for (NeighborSet::const_iterator na = nabrs.begin();
         ...
         if (nabr != mapped_box) {
            if ( nabr.getBlockId() == mapped_box.getBlockId() ) {
               has_overlap = nabr.getBox().intersects(mapped_box.getBox());
            }
            else {
               ...
               grid_geom.translateBox( nabr_box,
                                       ...
                                       nabr.getBlockId() );
               has_overlap = nabr_box.intersects(mapped_box.getBox());
            }
         }
      }
   }
   ...
   bool& remove_old_fine_level)
{
   ...
   if (compute_load_balanced_level_boxes) {
      ...
      for (hier::BoxList::Iterator itr(boxes_to_refine); itr; itr++, ++i) {
         hier::MappedBox unbalanced_mapped_box(*itr, i, 0);
   ...
   const hier::IntVector& fill_box_growth) const
{
   ...
   for (hier::PatchLevel::Iterator ip(tag_level); ip; ip++) {
      ...
      for (NeighborSet::const_iterator
         ...
         const hier::MappedBox &neighbor(*ni);
         hier::Box box = neighbor.getBox();
   ...
   const int tag_ln) const
{
   ...
   if ( nblocks == 1 ) {
      ...
      bounding_box =
         d_hierarchy->getMappedBoxLevel(tag_ln)->getGlobalBoundingBox(0);
       ...
       */
      hier::MappedBoxSet accumulated_mapped_boxes;
      for ( int bn=0; bn<nblocks; ++bn ) {
         ...
         hier::Box bounding_box(d_dim);
         bounding_box =
            d_hierarchy->getMappedBoxLevel(tag_ln)->getGlobalBoundingBox(bn);
         ...
         if ( !bounding_box.isEmpty() ) {
            ...
            accumulated_mapped_boxes.insert(
               new_mapped_box_level.getMappedBoxes().begin(),
               new_mapped_box_level.getMappedBoxes().end() );
         }

      }
      const hier::MappedBoxLevel &tag_mapped_box_level(tag_to_new.getBase());
       ...
       * neighbors.
       */
      tag_to_new.initialize( tag_mapped_box_level, new_mapped_box_level, tag_to_cluster_width );
      new_to_tag.initialize( new_mapped_box_level, tag_mapped_box_level, tag_to_cluster_width );
   ...
   if (new_mapped_box_level.getGlobalNumberOfBoxes() > 0) {
...

      {
         ...
         if (s_check_overflow_nesting == 'y') {
            ...
            bool nested = mblc_utils.baseNestsInHeadForMultiblock(
                  &locally_nested,
                  new_mapped_box_level,
                  tag_mapped_box_level,
                  hier::IntVector::getZero(d_dim),
       ...
       * generated boxes, deallocated if we didn't.
       */
 ...
 ***********************************************************************
 */
void GriddingAlgorithm::renumberBoxes(
   hier::MappedBoxLevel& new_mapped_box_level,
   hier::Connector& tag_to_new,
   hier::Connector& new_to_tag,
   ...
   const int tag_ln) const
{
    ...
    * candidate MappedBoxLevel that overlaps d_nesting_complement
    * violates nesting.
    */
   ...
   mblc_utils.computeInternalPartsForMultiblock(
      violator,
   ...
   for (hier::MappedBoxSet::const_iterator ni = candidate_mapped_boxes.begin();
      ...
      if (!addl_violators.isEmpty()) {
         ...
         if (candidate_eto_violator.find(cmb_non_per_id) !=
             ...
             * candidate_eto_violator, are non-nesting.  Leftovers are
   ...
   /*
    * Put back data into candidate_to_violator.
    */
   candidate_to_violator.swapInitialize(
      candidate_to_violator.getBase(),
      candidate_to_violator.getHead(),
...
void GriddingAlgorithm::computeProperNestingData(
   const int ln)
{
...
void GriddingAlgorithm::growBoxesWithinNestingDomain(
   hier::MappedBoxLevel& new_mapped_box_level,
   hier::Connector& tag_to_new,
   hier::Connector& new_to_tag,
   ...
   const int tag_ln) const
{
   ...
   if ( current_min_size >= min_size ) {
       ...
       */
      oca.shrinkConnectorWidth(
         tag_to_new,
         tag_to_new.getConnectorWidth() - min_size);
      oca.shrinkConnectorWidth(
         new_to_tag,
         new_to_tag.getConnectorWidth() - min_size);
      return;
   }
    ...
    * MappedBox, determine a sufficient view of the domain where it
    * can grow into.  This view includes domain boxes overlapping the
    * MappedBox minus parts removed to satisfy nesting requirements.
    */
   ...
   for (hier::MappedBoxSet::const_iterator ni = new_mapped_boxes.begin();
      ...
      if ( omb.getBox().numberCells() <= min_size ) {
      ...
      refined_domain_search_tree->findOverlapMappedBoxes(
         nesting_domain,
         omb.getBox(),
         omb.getBlockId(),
         new_mapped_box_level.getRefinementRatio());
      ...
      if ( new_to_nesting_complement.hasNeighborSet(omb.getId()) ) {
         const NeighborSet &neighbors(new_to_nesting_complement.getNeighborSet(omb.getId()));
         for ( NeighborSet::const_iterator na(neighbors.begin()); na!=neighbors.end(); ++na ) {
            nesting_domain.removeIntersections(na->getBox());
         }
      }
   ...
   /*
    * Create and use the mapping Connector.
    */
...
void GriddingAlgorithm::warnIfDomainTooSmallInPeriodicDir() const
{
