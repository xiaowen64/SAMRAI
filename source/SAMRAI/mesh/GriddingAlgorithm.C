   ...
   d_print_steps(false)
{
   ...
   tag_interior_variable_name(
      "GriddingAlgorithm__tag-interior");
   std::string
   tag_buffer_variable_name(
      "GriddingAlgorithm__tag-buffer");
   ...
   boost::shared_ptr<hier::BoxLevel> new_box_level)
{
   ...
   if (d_hierarchy->levelExists(new_ln + 1)) {
      ...
      if (d_check_proper_nesting) {
         ...
         required_nesting(
            dim,
            d_hierarchy->getProperNestingBuffer(new_ln));
   ...
   const int buffer_size) const
{
   ...
   for (hier::PatchLevel::iterator ip1(level->begin());
      ...
      pdat::CellIterator
      icend(
         pdat::CellGeometry::end(interior));
   ...
   for (hier::PatchLevel::iterator ip2(level->begin());
      ...
      pdat::CellIterator
      icend(
         pdat::CellGeometry::end(buf_tag_box));
   ...
   const bool for_building_finer) const
{
   ...
   if (for_building_finer) {
      ...
      const hier::IntVector
      sz(
         hier::IntVector::ceilingDivide(max_ghosts, den));
