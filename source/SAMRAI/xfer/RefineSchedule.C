   ...
   d_constructing_internal_schedule(false)
{
   ...
   FillSet dst_to_fill_on_src_proc(dim);
    ...
    */
   tbox::Pointer<BoxLevel> unused_unfilled_mapped_box_level;
   tbox::Pointer<Connector> unused_dst_to_unfilled;
   tbox::Pointer<BoxLevel> unused_unfilled_encon_box_level;
   ...
   const hier::Connector& encon_to_unfilled_encon)
{
    ...
    */
   d_coarse_interp_encon_schedule = new RefineSchedule(
      d_coarse_interp_encon_level,
      hiercoarse_level,
      next_coarser_ln - 1,
      hierarchy,
   ...
   const hier::Connector &dst_to_unfilled)
{
   ...
   hier::NeighborhoodSet dst_eto_coarse_interp(dim);
   hier::NeighborhoodSet coarse_interp_eto_unfilled(dim);
   ...
   for (hier::NeighborhoodSet::const_iterator ei = dst_eto_unfilled.begin();
      ...
      for (hier::BoxSet::OrderedConstIterator ni = dst_unfilled_parts.setBegin();
           ni != dst_unfilled_parts.setEnd(); ++ni) {
         ...
         if (sheared_coarse_interp_boxes.size() > 0) {
            ...
            NeighborSet& coarse_interp_nabrs =
               dst_eto_coarse_interp.getNeighborSet(dst_mapped_box_mbid, dim);
            ...
            for (hier::BoxList::Iterator b(sheared_coarse_interp_boxes); b; b++) {
               ...
               coarse_interp_eto_unfilled.insertNeighbor(
   ...
   dst_to_coarse_interp.swapInitialize(
      dst_to_unfilled.getBase(),
      coarse_interp_mapped_box_level,
      dst_to_coarse_interp_width,
      dst_eto_coarse_interp);

   coarse_interp_to_unfilled.swapInitialize(
      coarse_interp_mapped_box_level,
   ...
   const bool dst_is_coarse_interp_level )
{
   ...
   if (dst_is_coarse_interp_level) {
      ...
      const hier::BoxLevel& src_mapped_box_level =
         dst_to_src.getHead();
      if (hierarchy->getBoxLevel(next_coarser_ln + 1).getPointer() !=
   ...
   coarse_interp_level = new hier::PatchLevel(
         coarse_interp_mapped_box_level,
         hiercoarse_level->getGridGeometry(),
   ...
   overlaps) const
{
   ...
   for (hier::BoxSet::OrderedConstIterator ni = crse_mapped_boxes.setBegin();
        ni != crse_mapped_boxes.setEnd(); ++ni) {

      const hier::Box& crse_mapped_box = *ni;
      const hier::BoxSet& dst_nabrs =
       ...
       */
      for (hier::BoxSet::OrderedConstIterator na = dst_nabrs.setBegin();
           na != dst_nabrs.setEnd();
           ++na) {
         TBOX_ASSERT(na->isPeriodicImage() || na == dst_nabrs.setBegin());
   ...
   const Connector& coarse_to_unfilled)
{
   ...
   const hier::BoxSet& coarse_mapped_boxes =
      coarse_level->getBoxLevel()->getBoxes();
   for (hier::BoxSet::OrderedConstIterator ni = coarse_mapped_boxes.setBegin();
        ni != coarse_mapped_boxes.setEnd(); ++ni) {

      const hier::Box& coarse_mapped_box = *ni;
      const hier::BoxSet& fine_nabrs =
       ...
       */
      for (hier::BoxSet::OrderedConstIterator na = fine_nabrs.setBegin();
           na != fine_nabrs.setEnd();
           ++na) {
         TBOX_ASSERT(na->isPeriodicImage() || na == fine_nabrs.setBegin());
   ...
   const bool create_transactions)
{
   ...
   if (create_transactions) {
      ...
      for (FullNeighborhoodSet::const_iterator
         ...
         const hier::BoxSet& local_src_mapped_boxes = ei->second;
         ...
         for (hier::BoxSet::OrderedConstIterator
              ni = local_src_mapped_boxes.setBegin();
              ni != local_src_mapped_boxes.setEnd(); ++ni) {
   ...
   hier::BoxSet level_unfilled_boxes(dim);
   hier::BoxSet level_encon_unfilled_boxes(dim);
   hier::NeighborhoodSet dst_to_unfilled_nbrhood_set(dim);
   hier::NeighborhoodSet encon_to_unfilled_encon_nbrhood_set(dim);
   ...
   for (hier::NeighborhoodSet::const_iterator cf = dst_to_fill_edges.begin();
      ...
      const hier::BoxSet& fill_nabrs = cf->second;

      hier::BoxList fill_boxes_list(dim);
      for (hier::BoxSet::OrderedConstIterator bi = fill_nabrs.setBegin();
           bi != fill_nabrs.setEnd(); ++bi) {
      ...
      if (create_transactions) {
         ...
         if (dst_to_src_iter != dst_to_src_edges.end()) {
            const hier::BoxSet& src_mapped_boxes =
               dst_to_src_iter->second;

            for (hier::BoxSet::OrderedConstIterator
                 na = src_mapped_boxes.setBegin();
                 na != src_mapped_boxes.setEnd(); ++na) {
      ...
      if (unfilled_boxes_for_dst.size() > 0) {
         NeighborSet& unfilled_nabrs =
            dst_to_unfilled_nbrhood_set.getNeighborSet(dst_mapped_box_id, dim);
         ...
         for (hier::BoxList::Iterator bi(unfilled_boxes_for_dst);
            ...
            level_unfilled_boxes.insert(level_unfilled_boxes.setEnd(),
               unfilled_mapped_box);
            unfilled_nabrs.insert(unfilled_nabrs.setEnd(),
               unfilled_mapped_box);
   ...
   if (grid_geometry->hasEnhancedConnectivity()) {
      unfilled_encon_box_level = new BoxLevel(
            level_encon_unfilled_boxes,
            dst_mapped_box_level.getRefinementRatio(),
            dst_mapped_box_level.getGridGeometry(),
            dst_mapped_box_level.getMPI());
...
*/ 
   ...
   if (dst_to_src_iter != dst_to_src_edges.end()) {
      const hier::BoxSet& src_mapped_boxes = dst_to_src_iter->second;

      /*
       * If at enhanced connectivity, remove source box from container of
       * unfilled boxes
       */
      for (hier::BoxSet::OrderedConstIterator na = src_mapped_boxes.setBegin();
           na != src_mapped_boxes.setEnd(); ++na) {
...
//                  transformed_src_box);
   ...
   if (unfilled_encon_nbr_boxes.size() > 0) {
      ...
      if (find_encon_nabrs != dst_to_encon_nbrhood_set.end()) {
         ...
         for (NeighborSet::OrderedConstIterator de_iter = dst_encon_nabrs.setBegin();
              de_iter != dst_encon_nabrs.setEnd(); ++de_iter) {
            ...
            NeighborSet& unfilled_nabrs =
               encon_to_unfilled_encon_nbrhood_set.getNeighborSet(
                  encon_mapped_box_id, dst_mapped_box.getDim());
            ...
            if (unfilled_boxes.size() > 0) {
               ...
               for (hier::BoxList::ConstIterator bi(unfilled_boxes);
                  ...
                  level_encon_unfilled_boxes.insert(
                     level_encon_unfilled_boxes.setEnd(),
                     unfilled_encon_box);
   ...
   const hier::IntVector& fill_ghost_width)
{
   ...
   hier::BoxSet fill_mapped_boxes(dim);
   hier::NeighborhoodSet dst_eto_fill(dim);
...
void RefineSchedule::createEnconLevel(const hier::IntVector& fill_gcw)
{
   ...
   hier::BoxSet encon_mapped_boxes(dim);
   hier::NeighborhoodSet dst_to_encon_nabrs(dim);
                     ...
                     if (pgeom->getTouchesRegularBoundary()) {
                        ...
                        if (encon_test_list.size() > 0) {
                           ...
                           for (hier::BoxList::Iterator bi(encon_test_list);
                              ...
                              for (hier::BoxSetSingleBlockIterator
                               ...
                               */
                              dst_to_encon_nabrs.insertNeighbor(
                                 mapped_box_id,
                                 encon_mapped_box);
    ...
    */
   hier::BoxLevel encon_box_level(encon_mapped_boxes,
                                  d_dst_level->getRatioToLevelZero(),
                                  grid_geometry);
   ...
   const Connector& src_to_dst)
{
   ...
   hier::BoxSet tmp_fill_boxes(dim);
   ...
   for (hier::NeighborhoodSet::const_iterator ei = dst_eto_fill.begin();
      ...
      for (NeighborSet::OrderedConstIterator na = fill_nabrs.setBegin();
           na != fill_nabrs.setEnd(); ++na) {
         ...
         if (peer < comms + src_owners.size()) {
            ...
            while (ptr != peer->getRecvData() + peer->getRecvSize()) {
               ...
               for (size_t ii = 0; ii < num_fill_mapped_boxes; ++ii) {
                  ...
                  fill_boxes.insert(fill_boxes.setEnd(), tmp_dst_mapped_box);
               }
            }
   ...
   bool use_time_interpolation)
{
   ...
   if (s_extra_debug) {
      ...
      for (hier::BoxSet::OrderedConstIterator bi = fill_boxes.setBegin();
           bi != fill_boxes.setEnd(); ++bi) {
         tbox::plog << " " << *bi;
      }
      tbox::plog << std::endl;
   }
   ...
   if (is_singularity) {
      ...
      if (dst_mapped_box.getOwnerRank() == my_rank) {
         ...
         for (NeighborSet::OrderedConstIterator en = encon_nbrs.setBegin();
              en != encon_nbrs.setEnd(); ++en) {
         ...
         for (NeighborSet::OrderedConstIterator ni = encon_nbrs.setBegin();
              ni != encon_nbrs.setEnd(); ++ni) {
