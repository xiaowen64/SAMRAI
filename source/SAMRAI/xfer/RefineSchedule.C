   ...
   const hier::Connector& encon_to_unfilled_encon)
{
   ...
   Connector hiercoarse_to_coarse_interp_encon;
   {
      ...
      if (has_encon_to_hiercoarse && has_hiercoarse_to_encon) {
         ...
         if (*hierarchy->getBoxLevel(next_coarser_ln + 1) !=
         ...
         if (d_src_level->getBoxLevel()->getRefinementRatio() <=
             hierarchy->getBoxLevel(next_coarser_ln + 1)->
             getRefinementRatio()) {
   ...
   coarse_interp_encon_to_hiercoarse.initialize(
      *d_coarse_interp_encon_level->getBoxLevel(),
   ...
   const hier::Connector& dst_to_unfilled)
{
   ...
   for (hier::NeighborhoodSet::const_iterator ei = dst_eto_unfilled.begin();
      ...
      for (hier::BoxSet::const_iterator ni = dst_unfilled_parts.begin();
   ...
   overlaps) const
{
   ...
   for (hier::BoxSet::const_iterator ni = crse_mapped_boxes.begin();
       ...
       */
      for (hier::BoxSet::const_iterator na = dst_nabrs.begin();
           na != dst_nabrs.end();
           ++na) {
   ...
   const Connector& coarse_to_unfilled)
{
   ...
   const hier::BoxSet& coarse_mapped_boxes =
      coarse_level->getBoxLevel()->getBoxes();
   for (hier::BoxSet::const_iterator ni = coarse_mapped_boxes.begin();
       ...
       */
      for (hier::BoxSet::const_iterator na = fine_nabrs.begin();
           na != fine_nabrs.end();
           ++na) {
   ...
   const bool create_transactions)
{
   ...
   hier::BoxSet level_unfilled_boxes;
   ...
   const hier::BoxList& encon_fill_boxes)
{
   ...
   if (dst_to_src_iter != dst_to_src_edges.end()) {
       ...
       * unfilled boxes
       */
      for (hier::BoxSet::const_iterator na = src_mapped_boxes.begin();
           na != src_mapped_boxes.end(); ++na) {
...
void RefineSchedule::createEnconLevel(const hier::IntVector& fill_gcw)
{
   ...
   hier::BoxSet encon_mapped_boxes;
   ...
   bool use_time_interpolation)
{
   ...
   if (s_extra_debug) {
      ...
      for (hier::BoxSet::const_iterator bi = fill_boxes.begin();
