//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-2-1/source/algorithm/femutils/locally_active/LocallyActiveDataPatchBoundaryEdgeSum.C $
...
#ifndef included_algs_LocallyActiveDataPatchBoundaryEdgeSum_C
#define included_algs_LocallyActiveDataPatchBoundaryEdgeSum_C
...
template<int DIM> int 
   LocallyActiveDataPatchBoundaryEdgeSum<DIM>::s_instance_counter = 0;

template<int DIM> tbox::Array< tbox::Array<int> >
   LocallyActiveDataPatchBoundaryEdgeSum<DIM>::s_oedge_src_id_array = 
      tbox::Array< tbox::Array<int> >(0);
template<int DIM> tbox::Array< tbox::Array<int> >
   LocallyActiveDataPatchBoundaryEdgeSum<DIM>::s_oedge_dst_id_array = 
      tbox::Array< tbox::Array<int> >(0);
   ...
   const std::string& object_name)
{
   ...
   d_sum_transaction_factory = 
      new LocallyActiveDataOuteredgeSumTransactionFactory<DIM>();
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryEdgeSum<DIM>::registerSum(
   int edge_data_id)
{
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryEdgeSum<DIM>::setupSum(
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryEdgeSum<DIM>::computeSum() const
{
   d_level->allocatePatchData(d_oedge_src_data_set);
   d_level->allocatePatchData(d_oedge_dst_data_set);

   doLevelSum(d_level, d_level_mgr);
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryEdgeSum<DIM>::doLevelSum(
   tbox::Pointer<hier::PatchLevel<DIM> > level,
...
template<int DIM>
void LocallyActiveDataPatchBoundaryEdgeSum<DIM>::setInternalWorkDataActive(
   tbox::Pointer<hier::PatchLevel<DIM> > level,
   tbox::Pointer< hier::LocallyActiveDataPatchLevelManager<DIM> > level_mgr)
{
#ifdef DEBUG_CHECK_ASSERTIONS
