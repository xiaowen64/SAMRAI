//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-3-0/source/geometry/cartesian/operators/node/CartesianNodeFloatLinearRefine.C $
...
extern "C" {
// in cartrefine1d.f:
   void cartlinrefnodeflot1d_( const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               ...
                               const float*, float* );
// in cartrefine2d.f:
   void cartlinrefnodeflot2d_( const int&, const int&, const int&, const int&,
                               ...
                               const float*, float* );
// in cartrefine3d.f:
   void cartlinrefnodeflot3d_( const int&, const int&, const int&,
...
template<int DIM> CartesianNodeFloatLinearRefine<DIM>::CartesianNodeFloatLinearRefine()
...
template<int DIM> bool CartesianNodeFloatLinearRefine<DIM>::findRefineOperator(
   const tbox::Pointer< hier::Variable<DIM> >& var,
   const std::string &op_name) const
{
   const tbox::Pointer< pdat::NodeVariable<DIM,float> > cast_var(var);
...
CartesianNodeFloatLinearRefine<DIM>::getOperatorName() const
{
   return(d_name_id);
}

template<int DIM> int CartesianNodeFloatLinearRefine<DIM>::getOperatorPriority() const
{
   return(0);
}

template<int DIM> hier::IntVector<DIM> 
CartesianNodeFloatLinearRefine<DIM>::getStencilWidth() const {
   return(hier::IntVector<DIM>(0));
}

template<int DIM> void CartesianNodeFloatLinearRefine<DIM>::refine(
   hier::Patch<DIM>& fine, 
...
{
   tbox::Pointer< pdat::NodeData<DIM,float> >
   ...
   for (int d = 0; d < fdata->getDepth(); d++) {
      if (DIM == 1) {
	 cartlinrefnodeflot1d_(ifirstc(0),ilastc(0),
			       ifirstf(0),ilastf(0),
			       cilo(0),cihi(0),
			       filo(0),fihi(0),
			       ratio,
	 ...
	 cartlinrefnodeflot2d_(ifirstc(0),ifirstc(1),ilastc(0),ilastc(1),
	 ...
	 cartlinrefnodeflot3d_(ifirstc(0),ifirstc(1),ifirstc(2),
