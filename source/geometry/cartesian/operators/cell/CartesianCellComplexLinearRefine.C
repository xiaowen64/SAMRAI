//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-3-0/source/geometry/cartesian/operators/cell/CartesianCellComplexLinearRefine.C $
...
extern "C" {
// in cartrefine1d.f:
   void cartlinrefcellcplx1d_( const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
...
// in cartrefine2d.f:
   void cartlinrefcellcplx2d_( const int&, const int&, const int&, const int&,
                               ...
                               const dcomplex*, dcomplex* );
// in cartrefine3d.f:
   void cartlinrefcellcplx3d_( const int&, const int&, const int&,
...
template<int DIM> CartesianCellComplexLinearRefine<DIM>::CartesianCellComplexLinearRefine()
...
template<int DIM> bool CartesianCellComplexLinearRefine<DIM>::findRefineOperator(
...
CartesianCellComplexLinearRefine<DIM>::getOperatorName() const
{
   return(d_name_id);
}

template<int DIM> int CartesianCellComplexLinearRefine<DIM>::getOperatorPriority() const
{
   return(0);
}

template<int DIM> hier::IntVector<DIM> 
CartesianCellComplexLinearRefine<DIM>::getStencilWidth() const {
   return(hier::IntVector<DIM>(1));
}

template<int DIM> void CartesianCellComplexLinearRefine<DIM>::refine(
   hier::Patch<DIM>& fine, 
   ...
   const hier::IntVector<DIM>& ratio) const
{
   ...
   for (int d = 0; d < fdata->getDepth(); d++) {
      if (DIM == 1) {
	 cartlinrefcellcplx1d_(ifirstc(0),ilastc(0),
			       ifirstf(0),ilastf(0),
			       cilo(0),cihi(0),
			       filo(0),fihi(0),
			       ratio,
	 ...
	 cartlinrefcellcplx2d_(ifirstc(0),ifirstc(1),ilastc(0),ilastc(1),
	 ...
	 cartlinrefcellcplx3d_(ifirstc(0),ifirstc(1),ifirstc(2),
