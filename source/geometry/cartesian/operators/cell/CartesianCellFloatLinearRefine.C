//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-2-0/source/geometry/cartesian/operators/cell/CartesianCellFloatLinearRefine.C $
...
extern "C" {
// in cartrefine1d.f:
   void cartlinrefcellflot1d_( const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               ...
                               const float*, float* );
// in cartrefine2d.f:
   void cartlinrefcellflot2d_( const int&, const int&, const int&, const int&,
                               ...
                               const float*, float* );
// in cartrefine3d.f:
   void cartlinrefcellflot3d_( const int&, const int&, const int&,
...
template<int DIM> CartesianCellFloatLinearRefine<DIM>::CartesianCellFloatLinearRefine()
...
template<int DIM> bool CartesianCellFloatLinearRefine<DIM>::findRefineOperator(
...
CartesianCellFloatLinearRefine<DIM>::getOperatorName() const
{
   return(d_name_id);
}

template<int DIM> int CartesianCellFloatLinearRefine<DIM>::getOperatorPriority() const
{
   return(0);
}

template<int DIM> hier::IntVector<DIM> 
CartesianCellFloatLinearRefine<DIM>::getStencilWidth() const {
   return(hier::IntVector<DIM>(1));
}

template<int DIM> void CartesianCellFloatLinearRefine<DIM>::refine(
   hier::Patch<DIM>& fine, 
   ...
   const hier::IntVector<DIM>& ratio) const
{
   ...
   for (int d = 0; d < fdata->getDepth(); d++) {
      if (DIM == 1) {
	 cartlinrefcellflot1d_(ifirstc(0),ilastc(0),
			       ifirstf(0),ilastf(0),
			       cilo(0),cihi(0),
			       filo(0),fihi(0),
			       ratio,
	 ...
	 cartlinrefcellflot2d_(ifirstc(0),ifirstc(1),ilastc(0),ilastc(1),
	 ...
	 cartlinrefcellflot3d_(ifirstc(0),ifirstc(1),ifirstc(2),
