...
#include "vtkSamraiStructuredPointsReader.h"
...
// Read in a string from a XDR stream
...
// Read in integers from a XDR stream
...
// Read in floats from an XDR stream
...
// Read in doubles from an XDR stream
...
vtkSamraiStructuredPointsReader::vtkSamraiStructuredPointsReader()
{
   FileName = NULL;
...
const char *vtkSamraiStructuredPointsReader::GetClassName() 
{
      return "vtkSamraiStructuredPointsReader";
}

// Get the type of file 
int vtkSamraiStructuredPointsReader::GetFileFormat() 
{
   return FileFormat;
}

char *vtkSamraiStructuredPointsReader::GetCurrentVariableName() 
{
   return VariableName;
}

void vtkSamraiStructuredPointsReader::SetIndex(int index) 
{
   Index = index;
...
int vtkSamraiStructuredPointsReader::GetDepth(int i)
{
   return Depths[i];
}
...
void vtkSamraiStructuredPointsReader::SetCurrentVariableName(char *name) 
{
   int i,j,k;
   ...
   // If Index is greater than available reset to first 
   if (Index >= Depths[Variable]) 
   {
      Index = 0;
   }
...
//	 DisplayScalars[DisplayNumPatches] -> SetDataType(VTK_FLOAT);
	 
	 DisplayScalars[DisplayNumPatches] -> SetNumberOfTuples(size);
	 
	 // above here
	 
	 DisplayStructuredPoints[DisplayNumPatches] = DataStructuredPoints[patch];
	 
	 xdr_setpos(&Stream, StartingPosition[patch][Variable][Index]);
	 
	 float *data_ptr = 
	    ((vtkFloatArray *)DisplayScalars[DisplayNumPatches]) -> WritePointer(0,size);
	 ...
	 DisplayStructuredPoints[DisplayNumPatches] -> GetCellData() -> Update();
...
char **vtkSamraiStructuredPointsReader::GetVariableNames()
{
   return VariableNames;
}

char *vtkSamraiStructuredPointsReader::GetVariableName(int i)
{
   return VariableNames[i];
}

int vtkSamraiStructuredPointsReader::GetNumberOfVariables() 
{
   return NumVariables;
}


int vtkSamraiStructuredPointsReader::GetNumberOfPatches()
{
   return DisplayNumPatches;
}

vtkStructuredPoints *vtkSamraiStructuredPointsReader::GetPatch(int patch)

{
   return DisplayStructuredPoints[patch];
}

int vtkSamraiStructuredPointsReader::GetNumberOfRefinementLevels()
{
   return RefinementLevels;
}

int *vtkSamraiStructuredPointsReader::GetRefinement(int level)
{
   return &Refinement[level*3];
}

int vtkSamraiStructuredPointsReader::GetPatchLevel(int patch)
{
   return DataPatchLevel[patch];
}

int vtkSamraiStructuredPointsReader::GetNumberOfLevels()
{
   return NumLevels;
}

float *vtkSamraiStructuredPointsReader::GetBounds() 
{
   return Bounds;
}

int *vtkSamraiStructuredPointsReader::GetIndexBounds() 
{
   return IndexBounds;
}

float *vtkSamraiStructuredPointsReader::GetIndexScaling() 
{
   return IndexScale;
}


void   vtkSamraiStructuredPointsReader::ComputeBounds()
{
...
float *vtkSamraiStructuredPointsReader::GetScalarRange()
{
   return ScalarRange;
}

void vtkSamraiStructuredPointsReader::ComputeRange()
{
   int patch;
...
float *vtkSamraiStructuredPointsReader::GetSpacing()
{
   return Spacing;
}

float *vtkSamraiStructuredPointsReader::GetCenter()
{
   float *bounds;
...
void vtkSamraiStructuredPointsReader::Scale()
{
...
void vtkSamraiStructuredPointsReader::Execute()
{
...
void vtkSamraiStructuredPointsReader::FreeAllocated()
{
   if(Size) 
   {
      free(Size);
...
vtkStructuredPointsCollection *vtkSamraiStructuredPointsReader::GetCollection()
{
...
int vtkSamraiStructuredPointsReader::GetNumberOfPatchBoundaries() 
{
   return NumberOfPatchBoundaries;
}

vtkOutlineSource *vtkSamraiStructuredPointsReader::GetPatchBoundary(int patch)
{
   return PatchBoundaries[patch];
}

int vtkSamraiStructuredPointsReader::GetPatchBoundaryLevel(int patch)
{
