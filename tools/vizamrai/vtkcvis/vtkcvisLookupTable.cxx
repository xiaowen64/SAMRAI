...
// (from red to blue).
vtkcvisLookupTable::vtkcvisLookupTable(int sze, int ext)
{
...
vtkcvisLookupTable::~vtkcvisLookupTable()
{
  this->Table->Delete();
  this->Table = NULL;
}
...
// range value.
void vtkcvisLookupTable::SetTableRange(float r[2])
{
...
// Scalar values greater than maximum range value are clamped to maximum
// range value.
void vtkcvisLookupTable::SetTableRange(float rmin, float rmax)
{
...
void vtkcvisLookupTable::SetScale(int scale)
{
  if (this->Scale == scale)
    {
    return;
...
int vtkcvisLookupTable::Allocate(int sz, int ext)
{
...
void vtkcvisLookupTable::ForceBuild()
{
  int i, hueCase;
...
void vtkcvisLookupTable::GetColor(float v, float rgb[3])
{
...
float vtkcvisLookupTable::GetOpacity(float v)
{
  unsigned char *rgb8 = this->MapValue(v);
...
void vtkcvisLookupTableLogRange(float range[2], float logRange[2])
{
  float rmin = range[0];
  float rmax = range[1];
...
vtkIdType vtkcvisLookupTable::GetIndex(float v)
{
  ...
  if (this->Scale == VTK_SCALE_LOG10)
    ...
    float logRange[2];
    vtkcvisLookupTableLogRange(this->TableRange, logRange);
    shift = -logRange[0];
  ...
  // *****************************************************************
  // SGS Added this
  else if (this -> Scale == VTK_SCALE_SCHLIEREN)
    {
       float alpha = fabs(v);

       float max_alpha = fabs(this -> TableRange[0]);
       if ( fabs(this -> TableRange[1]) > max_alpha)
	   max_alpha = fabs(this -> TableRange[1]);
		       
       shift = 0;
       scale = maxIndex;
       v = exp(-this -> K * (alpha - this -> K0*max_alpha) / 
		   (this -> K1*max_alpha - this -> K0*max_alpha));
    }
  // SGS Added this
  // *****************************************************************
...
unsigned char *vtkcvisLookupTable::MapValue(float v)
{
  float maxIndex = this->NumberOfColors - 1;
  float shift, scale;
...
// of 8-bit chunks
template<class T>
void vtkcvisLookupTableMapData(vtkcvisLookupTable *self, T *input, 
                           ...
                           int inIncr, int outFormat)
{
  ...
  if ( (alpha=self->GetAlpha()) >= 1.0 ) //no blending required 
    ...
    if (self->GetScale() == VTK_SCALE_LOG10)
      ...
      float val;
      float logRange[2];
      vtkcvisLookupTableLogRange(range, logRange);
      shift = -logRange[0];
      if (logRange[1] <= logRange[0])
        {
        scale = VTK_LARGE_FLOAT;
    ...
    // *****************************************************************
    // SGS Added this
    else if (self -> GetScale() == VTK_SCALE_SCHLIEREN)
      {
      float alpha;

      float max_alpha = fabs(range[0]);
      if ( fabs(range[1]) > max_alpha)
	  max_alpha = fabs(range[1]);

      float v1 = -self -> GetK() / 
	 (self -> GetK1()*max_alpha - self -> GetK0()*max_alpha);

      float v2 = self -> GetK0()*max_alpha;
      ...
      if (outFormat == VTK_RGBA)
      {
        while (--i >= 0) 
          {
	  alpha = exp((fabs(*input) - v2) * v1);
          cptr = vtkLinearLookup(alpha, table, maxIndex, shift, scale); 
          *output++ = *cptr++;
          *output++ = *cptr++;
          *output++ = *cptr++;
          *output++ = *cptr++;     
...
void vtkcvisLookupTable::MapScalarsThroughTable2(void *input, 
                                             ...
                                             int outputFormat)
{
  switch (inputDataType)
    ...
    case VTK_BIT:
      ...
      vtkcvisLookupTableMapData(this,
    ...
    case VTK_CHAR:
      vtkcvisLookupTableMapData(this,static_cast<char *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
      
    case VTK_UNSIGNED_CHAR:
      vtkcvisLookupTableMapData(this,static_cast<unsigned char *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
      
    case VTK_SHORT:
      vtkcvisLookupTableMapData(this,static_cast<short *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
    break;
      
    case VTK_UNSIGNED_SHORT:
      vtkcvisLookupTableMapData(this,static_cast<unsigned short *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
      
    case VTK_INT:
      vtkcvisLookupTableMapData(this,static_cast<int *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
      
    case VTK_UNSIGNED_INT:
      vtkcvisLookupTableMapData(this,static_cast<unsigned int *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
      
    case VTK_LONG:
      vtkcvisLookupTableMapData(this,static_cast<long *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
      
    case VTK_UNSIGNED_LONG:
      vtkcvisLookupTableMapData(this,static_cast<unsigned long *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
      
    case VTK_FLOAT:
      vtkcvisLookupTableMapData(this,static_cast<float *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
      
    case VTK_DOUBLE:
      vtkcvisLookupTableMapData(this,static_cast<double *>(input),output,
                            numberOfValues,inputIncrement,outputFormat);
      break;
...
void vtkcvisLookupTable::SetNumberOfTableValues(vtkIdType number)
{
...
void vtkcvisLookupTable::SetTableValue(vtkIdType indx, float rgba[4])
{
...
void vtkcvisLookupTable::SetTableValue(vtkIdType indx, float r, float g, float b, 
                                   float a)
{
  float rgba[4];
...
void vtkcvisLookupTable::GetTableValue(vtkIdType indx, float rgba[4])
{
  unsigned char *_rgba;
...
// Return a rgba color value for the given index into the lookup table. Color
// components are expressed as [0,1] float values.
float *vtkcvisLookupTable::GetTableValue(vtkIdType indx)
{
  this->GetTableValue(indx, this->RGBA);
  return this->RGBA;
}

void vtkcvisLookupTable::PrintSelf(ostream& os, vtkIndent indent)
{
