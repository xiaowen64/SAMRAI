...
=========================================================================*/
#include "vtkcvisScalarBarActor.h"
...
vtkCxxSetObjectMacro(vtkcvisScalarBarActor,LabelTextProperty,vtkTextProperty);
vtkCxxSetObjectMacro(vtkcvisScalarBarActor,TitleTextProperty,vtkTextProperty);

//===========================================================================
// SGS added this
extern void vtkcvisLookupTableLogRange(float range[2], float logRange[2]);

//----------------------------------------------------------------------------
// Instantiate object with 64 maximum colors; 5 labels; %%-#6.3g label
// format, no title, and vertical orientation. The initial scalar bar
// size is (0.05 x 0.8) of the viewport size.
vtkcvisScalarBarActor::vtkcvisScalarBarActor()
{
...
void vtkcvisScalarBarActor::ReleaseGraphicsResources(vtkWindow *win)
{
...
//----------------------------------------------------------------------------
int vtkcvisScalarBarActor::RenderOverlay(vtkViewport *viewport)
{
...
//----------------------------------------------------------------------------
int vtkcvisScalarBarActor::RenderOpaqueGeometry(vtkViewport *viewport)
{
  int renderedSomething = 0;
  int i;
  int size[2];
  ...
  if (positionsHaveChanged ||
    ...
    //*******************************************************************
    // SGS Added this
    // If this is a log, plot along a log scale
    float mod_range[2];

    if (LookupTable && ((vtkLookupTable *)LookupTable) -> GetScale())
    {
       vtkcvisLookupTableLogRange(range, mod_range);
    }
    else
    {
       mod_range[0]=range[0];
...
//                           ((float)i /(numColors-1.0)));
      ...
      //*******************************************************************
      // SGS Added this
      float val = mod_range[0] + (mod_range[1] - mod_range[0])* 
	 ((float)i /(numColors-1.0));
      if (LookupTable && ((vtkLookupTable *)LookupTable) -> GetScale()) 
      {
	 val = pow(10,val);
      ...
      // Changed this to use different value
...
//----------------------------------------------------------------------------
void vtkcvisScalarBarActor::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkcvisScalarBarActor::ShallowCopy(vtkProp *prop)
{
  vtkcvisScalarBarActor *a = vtkcvisScalarBarActor::SafeDownCast(prop);
  if ( a != NULL )
    {
...
//----------------------------------------------------------------------------
void vtkcvisScalarBarActor::AllocateAndSizeLabels(int *labelSize, 
                                              int *size,
...
//    val = range[0] + (float)i/(this->NumberOfLabels-1) * (range[1]-range[0]);
    ...
    // SGS Added this
    val = mod_range[0] + 
       (float)i/(this->NumberOfLabels-1) * (mod_range[1]-mod_range[0]);
  ...
  if (this->NumberOfLabels)
    ...
    //*******************************************************************
    // SGS Added this
    targetWidth = (int)(targetWidth * 2.0 * LabelSize);
    targetHeight = (int)(targetHeight * 2.0 * LabelSize);
    // SGS Added this
    //*******************************************************************
...
//----------------------------------------------------------------------------
void vtkcvisScalarBarActor::SizeTitle(int *titleSize, 
                                  int *size, 
                                  vtkViewport *viewport)
{
  ...
  //***************************************************************************
  // SGS Added this
  targetWidth = (int)(targetWidth * 2.0 * TitleSize);
  targetHeight = (int)(targetHeight * 2.0 * TitleSize);
  // SGS Added this
  //***************************************************************************
...
void vtkcvisScalarBarActor::SetFontFamily(int val) 
{ 
...
#ifndef VTK_REMOVE_LEGACY_CODE
int vtkcvisScalarBarActor::GetFontFamily()
{ 
...
#ifndef VTK_REMOVE_LEGACY_CODE
void vtkcvisScalarBarActor::SetBold(int val)
{ 
...
#ifndef VTK_REMOVE_LEGACY_CODE
int vtkcvisScalarBarActor::GetBold()
{ 
...
#ifndef VTK_REMOVE_LEGACY_CODE
void vtkcvisScalarBarActor::SetItalic(int val)
{ 
...
#ifndef VTK_REMOVE_LEGACY_CODE
int vtkcvisScalarBarActor::GetItalic()
{ 
...
#ifndef VTK_REMOVE_LEGACY_CODE
void vtkcvisScalarBarActor::SetShadow(int val)
{ 
...
#ifndef VTK_REMOVE_LEGACY_CODE
int vtkcvisScalarBarActor::GetShadow()
{ 
