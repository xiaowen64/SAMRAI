//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-3-0/source/patchdata/operators/time_interpolate/cell/CellDoubleLinearTimeInterpolateOp.C $
...
extern "C" {
// in lintimint1d.f:
   void lintimeintcelldoub1d_( const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               const double&,
                               ...
                               double* );
// in lintimint2d.f:
   void lintimeintcelldoub2d_( const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               const int&, const int&,
                               ...
                               double* );
// in lintimint3d.f:
   void lintimeintcelldoub3d_( const int&, const int&, const int&,
...
template<int DIM> CellDoubleLinearTimeInterpolateOp<DIM>::CellDoubleLinearTimeInterpolateOp()
...
template<int DIM> bool CellDoubleLinearTimeInterpolateOp<DIM>::findTimeInterpolateOperator(
...
template<int DIM> void CellDoubleLinearTimeInterpolateOp<DIM>::timeInterpolate(
   ...
   const hier::PatchData<DIM>& src_data_new) const
{
   ...
   const CellData<DIM,double> *old_dat = 
      dynamic_cast<const CellData<DIM,double> *>(&src_data_old);
   const CellData<DIM,double> *new_dat =
      dynamic_cast<const CellData<DIM,double> *>(&src_data_new);
   CellData<DIM,double> *dst_dat =
      dynamic_cast<CellData<DIM,double> *>(&dst_data);
   ...
   for (int d = 0; d < dst_dat->getDepth(); d++) {
      if (DIM == 1) {
	 lintimeintcelldoub1d_(ifirst(0),ilast(0),
			       old_ilo(0),old_ihi(0),
			       new_ilo(0),new_ihi(0),
			       dst_ilo(0),dst_ihi(0),
			       tfrac,
	 ...
	 lintimeintcelldoub2d_(ifirst(0),ifirst(1),ilast(0),ilast(1),
	 ...
	 lintimeintcelldoub3d_(ifirst(0),ifirst(1),ifirst(2),
	 ...
	 TBOX_ERROR("CellDoubleLinearTimeInterpolateOp::TimeInterpolate DIM > 3 not supported" << std::endl);
      }
