//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-3-0/source/geometry/cartesian/operators/cell/CartesianCellComplexConservativeLinearRefine.C $
...
extern "C" {
// in cartrefine1d.f:
   void cartclinrefcellcplx1d_( const int&, const int&,
                                const int&, const int&,
                                const int&, const int&,
                                const int&, const int&,
                                ...
                                dcomplex*, dcomplex* );
// in cartrefine2d.f:
   void cartclinrefcellcplx2d_( const int&, const int&, const int&, const int&,
   ...
   void cartclinrefcellcplx3d_( const int&, const int&, const int&,
...
template<int DIM> CartesianCellComplexConservativeLinearRefine<DIM>::CartesianCellComplexConservativeLinearRefine()
...
template<int DIM> bool CartesianCellComplexConservativeLinearRefine<DIM>::findRefineOperator(
...
CartesianCellComplexConservativeLinearRefine<DIM>::getOperatorName() const
{
   return(d_name_id);
}

template<int DIM> int 
CartesianCellComplexConservativeLinearRefine<DIM>::getOperatorPriority() const
{
   return(0);
}

template<int DIM> hier::IntVector<DIM> 
CartesianCellComplexConservativeLinearRefine<DIM>::getStencilWidth() const {
   return(hier::IntVector<DIM>(1));
}

template<int DIM> void CartesianCellComplexConservativeLinearRefine<DIM>::refine(
   hier::Patch<DIM>& fine, 
   ...
   const hier::IntVector<DIM>& ratio) const
{
   ...
   tbox::Array<dcomplex> diff0(cgbox.numberCells(0)+1, memory_arena);
   pdat::CellData<DIM,dcomplex> slope0(cgbox, 1, tmp_ghosts, memory_arena);
   
   for (int d = 0; d < fdata->getDepth(); d++) {
      if (DIM == 1) {
	 cartclinrefcellcplx1d_(ifirstc(0),ilastc(0),
				ifirstf(0),ilastf(0),
				cilo(0),cihi(0),
				filo(0),fihi(0),
				ratio,
	 ...
	 tbox::Array<dcomplex> diff1(cgbox.numberCells(1)+1, memory_arena);
	 pdat::CellData<DIM,dcomplex> slope1(cgbox, 1, tmp_ghosts, memory_arena);
	 
	 cartclinrefcellcplx2d_(ifirstc(0),ifirstc(1),ilastc(0),ilastc(1),
	 ...
	 tbox::Array<dcomplex> diff2(cgbox.numberCells(2)+1, memory_arena);
	 pdat::CellData<DIM,dcomplex> slope2(cgbox, 1, tmp_ghosts, memory_arena);

	 cartclinrefcellcplx3d_(ifirstc(0),ifirstc(1),ifirstc(2),
