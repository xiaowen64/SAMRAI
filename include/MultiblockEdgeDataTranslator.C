...
MultiblockEdgeDataTranslator<DIM,TYPE>::MultiblockEdgeDataTranslator()
{
}
...
template <int DIM, class TYPE>
void MultiblockEdgeDataTranslator<DIM,TYPE>::translateAndCopyData(
      ...
      hier::MultiblockPatchHierarchy<DIM>::RotationIdentifier rotate)
{
   ...
   tbox::Pointer< EdgeData<DIM,TYPE> > dst = dst_patch.getPatchData(dst_id);
   tbox::Pointer< EdgeData<DIM,TYPE> > src = src_patch.getPatchData(src_id);
...
template<int DIM, class TYPE>
void MultiblockEdgeDataTranslator<DIM,TYPE>::translateAndCopyArrayData(
