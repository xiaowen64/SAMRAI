...
*************************************************************************
*                                                                       *
* The constructor for TimeRefinementIntegrator<DIM> checks for valid   *
   ...
   if (d_use_refined_timestepping) {
      if (max_levels > 1) {
         ...
         for (level_number = 1; level_number < max_levels; level_number++) {
            const hier::IntVector<DIM> ratio(d_gridding_algorithm->
...
template<int DIM> double TimeRefinementIntegrator<DIM>::initializeHierarchy()
{
   ...
   if (tbox::RestartManager::getManager()->isFromRestart()) {
      ...
      d_patch_hierarchy->getFromRestart(d_gridding_algorithm->getMaxLevels());
...
template<int DIM> double TimeRefinementIntegrator<DIM>::advanceHierarchy(
   const double dt,
...
TimeRefinementIntegrator<DIM>::initializeRefinedTimesteppingLevelData(
   const int level_number)
{
   ...
   if ( d_gridding_algorithm->levelCanBeRefined(level_number) ) {
      ...
      if ( d_patch_hierarchy->finerLevelExists(level_number) ) {
          ...
          */ 
         if ( d_gridding_algorithm->levelCanBeRefined(level_number+1) &&
...
template<int DIM> void
TimeRefinementIntegrator<DIM>::initializeSynchronizedTimesteppingLevelData(
   const int level_number)
{
#ifdef DEBUG_CHECK_ASSERTIONS
   TBOX_ASSERT( (level_number >= 0) &&
...
template<int DIM> void TimeRefinementIntegrator<DIM>::advanceRecursivelyForRefinedTimestepping(
   const int level_number, 
      ...
      if ( atRegridPoint(level_number) ) {
         ...
         if ( !lastLevelStep(level_number) 
            ...
            if (d_gridding_algorithm->errorEstimationUsesTimeIntegration() ) {
               ...
               for (int ln = level_number; ln <= finest_level_number; ln++) {
                  d_refine_level_integrator->resetTimeDependentData(
                     ...
                     d_level_sim_time[ln],
                     d_gridding_algorithm->levelCanBeRefined(ln));
               }
            }
            ...
            if (!d_gridding_algorithm->errorEstimationUsesTimeIntegration()) {
               ...
               int max_levels = d_gridding_algorithm->getMaxLevels();
               ...
               if (d_gridding_algorithm->getErrorCoarsenRatio() == 2) {
                  regrid_start_time = d_level_old_time;
               } else if (d_gridding_algorithm->getErrorCoarsenRatio() == 3) {
            ...
            d_gridding_algorithm->
               regridAllFinerLevels(d_patch_hierarchy,
                                    level_number,
                                    d_level_sim_time[level_number],
                                    d_tag_buffer,
                                    regrid_start_time,
...
template<int DIM> double TimeRefinementIntegrator<DIM>::advanceForSynchronizedTimestepping(
   const double end_time)
{
#ifdef DEBUG_CHECK_ASSERTIONS
   TBOX_ASSERT(end_time >= d_integrator_time);
   ...
   for (level_num = 0; level_num <= finest_level_number; level_num++) {
      ...
      tbox::Pointer< hier::BasePatchLevel<DIM> > patch_level =
...
template<int DIM> bool TimeRefinementIntegrator<DIM>::findNextDtAndStepsRemaining(
   const int level_number, 
...
template<int DIM> bool TimeRefinementIntegrator<DIM>::atRegridPoint(
...
template<int DIM> bool TimeRefinementIntegrator<DIM>::coarserLevelRegridsToo(
   const int level_number) const 
{
#ifdef DEBUG_CHECK_ASSERTIONS
   TBOX_ASSERT( (level_number >= 0) &&
...
template<int DIM> void TimeRefinementIntegrator<DIM>::putToDatabase(
...
template<int DIM> void TimeRefinementIntegrator<DIM>::getFromInput(
   tbox::Pointer<tbox::Database> db,
   bool is_from_restart)
{
   ...
   if (is_from_restart) {
      ...
      if (db->keyExists("tag_buffer")) {
         ...
         d_tag_buffer.resizeArray(d_gridding_algorithm->getMaxLevels());
         for (level_number = 0; 
              level_number < d_gridding_algorithm->getMaxLevels();
              level_number++) {
...
template<int DIM> void TimeRefinementIntegrator<DIM>::getFromRestart()
{
