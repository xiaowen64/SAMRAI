//
// File:        $URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-3-0/source/algorithm/hyperbolic/HyperbolicPatchStrategy.h $
...
/**
 * Class HyperbolicPatchStrategy<DIM> is an abstract base class defining the 
 * interface between an HyperbolicLevelIntegrator<DIM> object and operations 
 ...
 * and the HyperbolicLevelIntegrator<DIM> to work together, the concrete 
...
template<int DIM> class HyperbolicPatchStrategy 
:
...
public:
   /**
    * Blank constructor for HyperbolicPatchStrategy<DIM>.
    */
   ...
   virtual void registerModelVariables(
      HyperbolicLevelIntegrator<DIM>* integrator) = 0;
   ...
   virtual void setupLoadBalancer(
      HyperbolicLevelIntegrator<DIM>* integrator,
    ...
    */
   virtual void computeFluxesOnPatch(hier::Patch<DIM>& patch,
                                     const double time,
                                     const double dt) = 0;
    ...
    */
   virtual void 
   conservativeDifferenceOnPatch(hier::Patch<DIM>& patch,
                                 const double time, 
                                 const double dt,
    ...
    */
   virtual void 
   preprocessAdvanceLevelState(const tbox::Pointer< hier::PatchLevel<DIM> >& level,
                               double current_time,
                               double dt,
                               bool first_step,
                               bool last_step,
    ...
    */
   virtual void 
   postprocessAdvanceLevelState(const tbox::Pointer< hier::PatchLevel<DIM> >& level,
                                double current_time,
                                double dt,
                                bool first_step,
                                bool last_step,
