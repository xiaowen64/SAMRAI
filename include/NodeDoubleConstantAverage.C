//
// File:	NodeDoubleConstantAverage.C
...
#ifndef included_pdat_NodeDoubleConstantAverage_C
#define included_pdat_NodeDoubleConstantAverage_C
...
template<int DIM> NodeDoubleConstantAverage<DIM>::NodeDoubleConstantAverage()
...
template<int DIM> bool NodeDoubleConstantAverage<DIM>::findCoarsenOperator(
...
NodeDoubleConstantAverage<DIM>::getOperatorName() const
{
   return(d_name_id);
}

template<int DIM> int NodeDoubleConstantAverage<DIM>::getOperatorPriority() const
{
   return(0);
}

template<int DIM> hier::IntVector<DIM> 
NodeDoubleConstantAverage<DIM>::getStencilWidth() const {
   return(hier::IntVector<DIM>(0));
}

template<int DIM> void NodeDoubleConstantAverage<DIM>::coarsen(
   ...
   const hier::IntVector<DIM>& ratio) const 
{
   ...
   for (int d = 0; d < cdata->getDepth(); d++) {
	 ...
	 TBOX_ERROR("NodeDoubleConstantRefine::coarsen DIM > 3 not supported" << endl);
      }
   }
}
