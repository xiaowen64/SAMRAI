//
// File:	NodeFloatConstantAverage.C
...
#ifndef included_pdat_NodeFloatConstantAverage_C
#define included_pdat_NodeFloatConstantAverage_C
...
template<int DIM> NodeFloatConstantAverage<DIM>::NodeFloatConstantAverage()
...
template<int DIM> bool NodeFloatConstantAverage<DIM>::findCoarsenOperator(
...
NodeFloatConstantAverage<DIM>::getOperatorName() const
{
   return(d_name_id);
}

template<int DIM> int NodeFloatConstantAverage<DIM>::getOperatorPriority() const
{
   return(0);
}

template<int DIM> hier::IntVector<DIM> 
NodeFloatConstantAverage<DIM>::getStencilWidth() const {
   return(hier::IntVector<DIM>(0));
}

template<int DIM> void NodeFloatConstantAverage<DIM>::coarsen(
   ...
   const hier::IntVector<DIM>& ratio) const 
{
   ...
   for (int d = 0; d < cdata->getDepth(); d++) {
	 ...
	 TBOX_ERROR("NodeFloatConstantRefine::coarsen DIM > 3 not supported" << endl);
      }
   }
}
