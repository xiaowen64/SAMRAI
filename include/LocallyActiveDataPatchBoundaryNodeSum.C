...
#ifndef included_algs_LocallyActiveDataPatchBoundaryNodeSum_C
#define included_algs_LocallyActiveDataPatchBoundaryNodeSum_C
...
template<int DIM> int 
   LocallyActiveDataPatchBoundaryNodeSum<DIM>::s_instance_counter = 0;

template<int DIM> tbox::Array< tbox::Array<int> > 
   LocallyActiveDataPatchBoundaryNodeSum<DIM>::s_onode_src_id_array =
      tbox::Array< tbox::Array<int> >(0);
template<int DIM> tbox::Array< tbox::Array<int> > 
   LocallyActiveDataPatchBoundaryNodeSum<DIM>::s_onode_dst_id_array =
      tbox::Array< tbox::Array<int> >(0);
   ...
   const std::string& object_name)
{
   ...
   d_sum_transaction_factory = 
      new LocallyActiveDataOuternodeSumTransactionFactory<DIM>();
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryNodeSum<DIM>::registerSum(
   int node_data_id)
{
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryNodeSum<DIM>::setupSum(
   ...
   tbox::Pointer< hier::LocallyActiveDataPatchLevelManager<DIM> > level_mgr)
{
   ...
   if (d_hierarchy_setup_called) {
      ...
      xfer::LocallyActiveDataRefineAlgorithm<DIM> single_level_sum_algorithm;
   ...
   const int finest_level)
{
   ...
   if (d_level_setup_called) {
      ...
      d_level_mgr.resizeArray(num_levels);
      d_cfbdry_tmp_level_mgr.resizeArray(num_levels);
      ...
      for (int ln = d_coarsest_level; ln <= d_finest_level; ln++) {
         ...
         setInternalWorkDataActive(level,
      ...
      // next finer level
      xfer::LocallyActiveDataRefineAlgorithm<DIM> cfbdry_copy_algorithm;

      // Communication algorithm for coarsening outernode values on 
      // each finer level to node data on next coarser level
      xfer::LocallyActiveDataCoarsenAlgorithm<DIM> sync_coarsen_algorithm(false);
      ...
      for (int ln = d_coarsest_level+1; ln <= d_finest_level; ln++) {
         ...
         d_cfbdry_tmp_level_mgr[fine_level_num] = 
            new hier::LocallyActiveDataPatchLevelManager<DIM>(
               d_cfbdry_tmp_level[fine_level_num]);
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryNodeSum<DIM>::computeSum(
   const bool fill_hanging_nodes) const
{
   ...
   if (d_level_setup_called) {
      ...
      doLevelSum(d_level, d_level_mgr[0]);
      ...
      for (ln = d_coarsest_level; ln <= d_finest_level; ln++) {
        ...
        doLevelSum(level, d_level_mgr[ln]);
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryNodeSum<DIM>::doLevelSum(
   tbox::Pointer<hier::PatchLevel<DIM> > level,
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryNodeSum<DIM>::doLocalCoarseFineBoundarySum(
   tbox::Pointer<hier::PatchLevel<DIM> > fine_level,
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryNodeSum<DIM>::copyNodeToOuternodeOnLevel(
   tbox::Pointer<hier::PatchLevel<DIM> > level,
   tbox::Pointer< hier::LocallyActiveDataPatchLevelManager<DIM> > level_mgr,
   const tbox::Array<int>& node_data_id,
...
template<int DIM> 
void LocallyActiveDataPatchBoundaryNodeSum<DIM>::copyOuternodeToNodeOnLevel(
   tbox::Pointer<hier::PatchLevel<DIM> > level,
   tbox::Pointer< hier::LocallyActiveDataPatchLevelManager<DIM> > level_mgr,
   const tbox::Array<int>& onode_data_id,
...
*************************************************************************
*                                                                       *
* Private member function to set up active data with level managers.    *
*                                                                       *
*************************************************************************
*/

template<int DIM>
void LocallyActiveDataPatchBoundaryNodeSum<DIM>::setInternalWorkDataActive(
   tbox::Pointer<hier::PatchLevel<DIM> > level,
   tbox::Pointer< hier::LocallyActiveDataPatchLevelManager<DIM> > level_mgr)
{
#ifdef DEBUG_CHECK_ASSERTIONS
